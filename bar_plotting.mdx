...
[na](https://www.tradingview.com/pine-script-reference/v5/#var_na), no
bar is plotted.
...
![image](@assets/images/BarPlotting-Plotcandle-1.png)
...
-   We show the script\'s plot after having used \"Visual Order/Bring to
    Front\" from the script\'s \"More\" menu. This causes our script\'s
    candles to appear on top of the chart\'s candles.
...
-   The script will only display candles when two conditions are met:
    ...
    >     value on timeframes higher or equal to 1D.
    > -   The
...
-   We use
    ...
    to declare our `UP_COLOR` and `DN_COLOR` color constants on bar zero
    ...
    only do so in one place.

-   We create a lighter transparency for the body of our candles in the
    `bodyColor` variable initialization, so they don\'t obstruct the
    chart\'s candles.
...
[![image](/images/logo/TradingView_Logo_Block.svg){.align-center
width="200px"}](https://www.tradingview.com/)
