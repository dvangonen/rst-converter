...
`built-in variables <PageBuiltInFunctions_BuiltInVariables>`. The ones we cover here allow scripts to access information
relating to:
...
`repainting <PageRepainting>`. See the
...
`history-referencing operator <PageOperators_HistoryReferencingOperator>` can be used to refer to past values of the built-in
...
See the page on `Timeframes <PageTimeframes>` for more information.
...
-   The
    ...
    [ticker.new()](https://www.tradingview.com/pine-script-reference/v5/#fun_ticker%7Bdot%7Dnew).
-   `Session state built-ins <PageSessions_SessionStates>` provide information on the trading session a bar belongs
    to.
