...
sidebar-title: Error messages
page-title:  Error messages
...
statement](https://www.tradingview.com/pine-script-reference/v5/#kw_if)
...
statements](https://www.tradingview.com/pine-script-reference/v5/#kw_if)).
...
`request.security` after compilation:
...
adding a
[special attribute](/pine-script-docs/language/script-structure#version) in the first line. You\'ll get
...
use of series in branches of conditional statements (`if`, `iff` or
...
`max_bars_back` parameter to the script\'s `indicator` or `strategy`
function:
...
built-in **function** (`vwma` in our example), you may use the
...
[drawings](/pine-script-docs/concepts/lines-and-boxes#historical-buffer-and-max-bars-back).
