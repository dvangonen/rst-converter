...
Pine Script™ Maps are collections that store elements in *key-value
pairs*. They allow scripts to collect multiple value references
associated with unique identifiers (keys).

Unlike [arrays](/pine-script-docs/language/arrays) and
[matrices](/pine-script-docs/language/matrices), maps are
considered *unordered* collections. Scripts quickly access a map\'s
values by referencing the keys from the key-value pairs put into them
rather than traversing an internal index.

A map\'s keys can be of any *fundamental type*, and its values can be of
any built-in or
[user-defined](/pine-script-docs/language/type-system#user-defined-types) type. Maps cannot directly use other *collections* (maps,
[arrays](/pine-script-docs/language/arrays), or
[matrices](/pine-script-docs/language/matrices)) as values, but
they can hold [UDT](/pine-script-docs/language/type-system#user-defined-types) instances containing these data structures within their
fields. See
[this section](/pine-script-docs/language/maps#maps-of-other-collections) for more information.

As with other collections, maps can contain up to 100,000 elements in
total. Since each key-value pair in a map consists of two elements (a
unique *key* and its associated *value*), the maximum number of
key-value pairs a map can hold is 50,000.
...
Where `<keyType, valueType>` is the map\'s
[type template](/pine-script-docs/language/type-system#type-templates) that declares the types of keys and values it will contain,
and the `<expression>` returns either a map instance or `na`.
...
[type template](/pine-script-docs/language/type-system#type-templates) to tell the compiler that the variable can accept maps with
`keyType` keys and `valueType` values.

For example, this line of code declares a new `myMap` variable that can
...
When the `<expression>` is not `na`, the compiler does not require
explicit type declaration, as it will infer the type information from
the assigned map object.
...
instances on each script iteration until explicitly reassigned.

For example, this script declares a `colorMap` variable assigned to a
map that holds pairs of
[string](https://www.tradingview.com/pine-script-reference/v5/#type_string)
keys and
[color](https://www.tradingview.com/pine-script-reference/v5/#type_color)
values on the first chart bar. The script displays an `oscillator` on
the chart and uses the values it
[put](https://www.tradingview.com/pine-script-reference/v5/#fun_map.put)
into the `colorMap` on the *first* bar to color the plots on *all* bars:

![image](@assets/language/Maps-Declaring-a-map-Using-var-and-varip-keywords-1.png)
...
[matrices](/pine-script-docs/language/matrices), or maps) of
these types.
:::
...
[contains](https://www.tradingview.com/pine-script-reference/v5/#fun_map.contains)
...
the map and plots its value on the chart:

![image](@assets/language/Maps-Reading-and-writing-Putting-and-getting-key-value-pairs-1.png)
...
Note that:
    ...
    -   This script replaces the values associated with the "Rising",
        ...
        [data.put()](https://www.tradingview.com/pine-script-reference/v5/#fun_map.put)
...
Similar to working with other collections, when putting a value of a
*special type*
...
`value` points to that same object without copying it. Modifying the
...
Each change to the object with the \"B\" key affects the one referenced
by the \"A\" key, as shown by the candle plot of the \"A\" object\'s
fields:

![image](@assets/language/Maps-Reading-and-writing-Putting-and-getting-key-value-pairs-2.png)
...
Note that:
    ...
    -   This script would behave differently if it passed a copy of
        `myData` into each
        [myMap.put()](https://www.tradingview.com/pine-script-reference/v5/#fun_map.put)
        call. For more information, see
        [this](/pine-script-docs/language/objects#copying-objects) section of our User Manual\'s page on
...
and
[map.values()](https://www.tradingview.com/pine-script-reference/v5/#fun_map.values).
...
![image](@assets/language/Maps-Reading-and-writing-Inspecting-keys-and-values-1.png)
...
Note that:
    ...
    -   The value with the \"First\" key is a
...
Below, we\'ve added a line of code to
[put](https://www.tradingview.com/pine-script-reference/v5/#fun_map.put)
...
new pair into the map *after* the one with the \"Third\" key, the
pair\'s key and value are still second in the `keys` and `values` arrays
since the key was already present in `m` *before* the change:

![image](@assets/language/Maps-Reading-and-writing-Inspecting-keys-and-values-2.png)
...
The elements in a
[map.values()](https://www.tradingview.com/pine-script-reference/v5/#fun_map.values)
array point to the same values as the map `id`. Consequently, when the
map\'s values are of *reference types*, including
[line](https://www.tradingview.com/pine-script-reference/v5/#type_line),
[linefill](https://www.tradingview.com/pine-script-reference/v5/#type_linefill),
[box](https://www.tradingview.com/pine-script-reference/v5/#type_box),
[polyline](https://www.tradingview.com/pine-script-reference/v5/#type_polyline),
...
[table](https://www.tradingview.com/pine-script-reference/v5/#type_table),
[chart.point](https://www.tradingview.com/pine-script-reference/v5/#type_chart.point),
or [UDTs](/pine-script-docs/language/type-system#user-defined-types), modifying the instances referenced by the
[map.values()](https://www.tradingview.com/pine-script-reference/v5/#fun_map.values)
array will also affect those referenced by the map `id` since the
contents of both collections point to identical objects.
:::
...
[contained](/pine-script-docs/language/maps#map-contains) the `key`. Otherwise, it returns
[na](https://www.tradingview.com/pine-script-reference/v5/#var_na).
...
While a
[shallow copy](/pine-script-docs/language/maps#shallow-copies) will suffice when copying maps that have values of a
...
Note that:
    ...
    -   The `deepCopy()` method loops through the `original` map,
        ...
        [putting](/pine-script-docs/language/maps#putting-and-getting-key-value-pairs) key-value pairs containing the copies into a
...
[user-defined type](/pine-script-docs/language/type-system#user-defined-types) that contains collections within its fields.
