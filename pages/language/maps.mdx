...
Unlike `arrays <PageArrays>` and
`matrices <PageMatrices>`, maps are
...
any built-in or
`user-defined <PageTypeSystem_UserDefinedTypes>` type. Maps cannot directly use other *collections* (maps,
`arrays <PageArrays>`, or
`matrices <PageMatrices>`) as values, but
they can hold `UDT <PageTypeSystem_UserDefinedTypes>` instances containing these data structures within their
fields. See
`this section <PageMaps_MapsOfOtherCollections>` for more information.
...
`type template <PageTypeSystem_TypeTemplates>` that declares the types of keys and values it will contain,
...
`type template <PageTypeSystem_TypeTemplates>` to tell the compiler that the variable can accept maps with
...
`matrices <PageMatrices>`, or maps) of
these types.
:::
...
Note that:
    ...
    -   Replacing the pairs in a map does not change the internal
        *insertion order* of its keys. We discuss this further in the
        `next section <PageMaps_ReadingAndWriting_InspectingKeysAndValues>`.
...
or a
`user-defined type <PageTypeSystem_UserDefinedTypes>` into a map, it\'s important to note the inserted pair\'s
...
Note that:
    ...
    -   This script would behave differently if it passed a copy of
        ...
        `this <PageObjects_CopyingObjects>` section of our User Manual\'s page on
        `objects <PageObjects>`.
...
and
[map.values()](https://www.tradingview.com/pine-script-reference/v5/#fun_map.values)
functions always return `arrays <PageArrays>` with their elements ordered based on the `id` map\'s
insertion order.
...
`removes <PageMaps_ReadingAndWriting_RemovingKeyValuePairs>` the key beforehand.
...
[table](https://www.tradingview.com/pine-script-reference/v5/#type_table),
[chart.point](https://www.tradingview.com/pine-script-reference/v5/#type_chart.point),
or `UDTs <PageTypeSystem_UserDefinedTypes>`, modifying the instances referenced by the
[map.values()](https://www.tradingview.com/pine-script-reference/v5/#fun_map.values)
