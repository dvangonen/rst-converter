...
[arrays](/pine-script-docs/language/arrays), all matrix elements
must be of the same [type](/pine-script-docs/language/type-system#types), which can be a
[built-in](https://www.tradingview.com/pine-script-docs/en/v5/language/Type_system.html#id2)
or a
[user-defined type](/pine-script-docs/language/type-system#user-defined-types).
...
[arrays](/pine-script-docs/language/arrays), the total number of
...
Where `<type>` is a
[type template](/pine-script-docs/language/type-system#type-templates) for the matrix that declares the type of values it will
...
types or
[user-defined types](/pine-script-docs/language/type-system#user-defined-types) that exclusively contain within their fields these types or
collections ([arrays](/pine-script-docs/language/arrays),
matrices, or [maps](/pine-script-docs/language/maps)) of these
types.
:::
...
[retrieved](/pine-script-docs/language/matrices#retrieving) from a matrix of
[line](https://www.tradingview.com/pine-script-reference/v5/#type_line),
[linefill](https://www.tradingview.com/pine-script-reference/v5/#type_linefill),
[box](https://www.tradingview.com/pine-script-reference/v5/#type_box),
[polyline](https://www.tradingview.com/pine-script-reference/v5/#type_polyline),
...
[table](https://www.tradingview.com/pine-script-reference/v5/#type_table),
[chart.point](https://www.tradingview.com/pine-script-reference/v5/#type_chart.point),
or [UDT](/pine-script-docs/language/type-system#user-defined-types)
instances behave as shallow copies, the elements of matrices containing
such types reference the same objects as the
[arrays](/pine-script-docs/language/arrays) inserted into them.
...
[section above](/pine-script-docs/language/matrices#inserting):
...
[previous example](/pine-script-docs/language/matrices#removing), which swap the first and last rows of `m` and display the
...
[insert](/pine-script-docs/language/matrices#inserting) the new array at the desired `row/column` and
...
[nested for loops](/pine-script-docs/language/matrices#for) to assign all elements to copies of the originals. When the
...
matrix when
[multiplied](/pine-script-docs/language/matrices#matrix-mult) by the original. Inverses have utility in various matrix
...
Note that:
    ...
    -   The
        [determinants](/pine-script-docs/language/matrices#matrix-det) of full-rank square matrices are nonzero, and such
        matrices have
        [inverses](/pine-script-docs/language/matrices#matrix-inv-and-matrix-pinv). Conversely, the
        [determinant](https://www.tradingview.com/pine-script-reference/v5/#fun_matrix.det)
...
When using
[matrix.sum() and matrix.diff()](/pine-script-docs/language/matrices#matrix-sum-and-matrix-diff) functions, the `id1` and `id2` matrices must have the same
...
When using
[matrix.mult()](/pine-script-docs/language/matrices#matrix-mult) to multiply an `id1` matrix by an `id2` matrix or array, the
