...
sidebar-title: Variable declarations
page-title: Language / Variable declarations
...
Variables are `identifiers <PageIdentifiers>` that hold values. They must be *declared* in your code
...
-   \<declaration_mode\> is the variable\'s
    `declaration mode <PageVariableDeclarations_DeclarationModes>`. It can be
...
-   \<structure\> can be an
    ...
    *structure*.
-   \<tuple_declaration\> is a comma-separated list of variable names
    enclosed in square brackets (`[]`), e.g.,
    `[ma, upperBand, lowerBand]`.

These are all valid variable declarations. The last one requires four
lines:
...
:::warning[Notice!]
The above statements all contain the `=` assignment operator because
they are **variable declarations**. When you see similar lines using the
`:= <PageOperators_ReassignmentOperator>`
reassignment operator, the code is **reassigning** a value to a variable
that was **already declared**. Those are **variable reassignments**. Be
sure you understand the distinction as this is a common stumbling block
for newcomers to Pine Script™. See the next
`Variable reassignment <PageVariableDeclarations_VariableReassignment>` section for details.
:::
...
[na](https://www.tradingview.com/pine-script-reference/v5/#var_na) is a
...
[ta.bb()](https://www.tradingview.com/pine-script-reference/v5/#fun_ta%7Bdot%7Dbb)
...
-   On every bar, the
    ...
    `:= <PageOperators_ReassignmentOperator>` reassignment operator.
-   If we did not use the
    `:= <PageOperators_ReassignmentOperator>` reassignment operator, the effect would be to initialize
...
its *type qualifier* (see the page on Pine Script™\'s
...
`declaration mode <PageVariableDeclarations_DeclarationModes>` determines how new values assigned to a variable will be
saved.
...
![image](@assets/language/VariableDeclarations-GreenBarsCount.png)
...
drawings more efficiently. Suppoose we want to extend the last bar\'s
...
[var](https://www.tradingview.com/pine-script-reference/v5/#kw_var) if
...
`bar states <PageBarStates>`.

The
[varip](https://www.tradingview.com/pine-script-reference/v5/#kw_varip)
keyword can be used to declare variables that escape the *rollback
process*, which is explained in the page on Pine Script™\'s
`execution model <PageExecutionModel>`.
...
[na](https://www.tradingview.com/pine-script-reference/v5/#var_na) at
each execution, the `updateNo + 1` expression yields
[na](https://www.tradingview.com/pine-script-reference/v5/#var_na), so
