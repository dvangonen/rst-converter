...
[profile](/pine-script-docs/profiling-and-optimization#pine-profiler) a script\'s runtime performance and identify ways to modify
...
executions with the
[Pine Profiler](/pine-script-docs/profiling-and-optimization#pine-profiler) and explains some ways programmers can modify their code to
[optimize](/pine-script-docs/profiling-and-optimization#optimization) runtime performance.

## Pine Profiler 

Before diving into
[optimization](/pine-script-docs/profiling-and-optimization#optimization), it\'s prudent to evaluate a script\'s runtime and pinpoint
...
[interpretation](/pine-script-docs/profiling-and-optimization#interpreting-profiled-results) while profiling:
...
Note that:
    ...
    -   Profiler results **do not** appear for script declaration
        ...
        *repetitive code* that the compiler optimizes during
        translation. See
        [this section](/pine-script-docs/profiling-and-optimization#insignificant-unused-and-redundant-code) for more information.
...
[section below](/pine-script-docs/profiling-and-optimization#interpreting-profiled-results) explains different types of code the Profiler analyzes and
...
[extra calculations](/pine-script-docs/profiling-and-optimization#alook-into-the-profilers-inner-workings) required to collect performance data. As such, a script\'s
...
[single-line result](/pine-script-docs/profiling-and-optimization#single-line-results):
...
[this section](/pine-script-docs/profiling-and-optimization#alook-into-the-profilers-inner-workings) for more information.
...
[user-defined functions](/pine-script-docs/language/user-defined-functions) **will not** include the time spent on the `request.*()`
calls. See the
[section below](/pine-script-docs/profiling-and-optimization#when-requesting-other-contexts) to learn more.
:::
...
[line](/pine-script-docs/profiling-and-optimization#single-line-results) or
[block](/pine-script-docs/profiling-and-optimization#code-block-results) represents the results from executing the code in *all
...
value into the
[array](https://www.tradingview.com/pine-script-reference/v5/#type_array),
and it [plots](/pine-script-docs/concepts/plots) the [array\'s
...
*outputs* ([plots](/pine-script-docs/concepts/plots),
...
[plots](/pine-script-docs/concepts/plots) the
[close](https://www.tradingview.com/pine-script-reference/v5/#var_close)
...
from **both**
[function calls](/pine-script-docs/profiling-and-optimization#user-defined-function-calls):
...
[single line](/pine-script-docs/profiling-and-optimization#single-line-results) of code, where `_startX` represents the starting time for
the `lineX` line:
...
[code blocks](/pine-script-docs/profiling-and-optimization#code-block-results). The difference is that the `registerPerf()` call maps the
...
excluding any
[insignificant, unused, or redundant code](/pine-script-docs/profiling-and-optimization#insignificant-unused-and-redundant-code), with the aforementioned **internal** functions to collect
...
[code blocks](/pine-script-docs/profiling-and-optimization#code-block-results) also occurs at this stage, as the Profiler cannot
...
[here](/pine-script-docs/profiling-and-optimization#code-block-results).
...
[section below](/pine-script-docs/profiling-and-optimization#repetitive-profiling) for more information.
:::
...
[queues](/pine-script-docs/language/arrays#using-an-array-as-aqueue) pseudorandom values with a constant seed through an
[array](https://www.tradingview.com/pine-script-reference/v5/#type_array)
...
[profile it](/pine-script-docs/profiling-and-optimization#profiling-ascript) to gauge its performance and identify the **critical code
...
[Profiler](/pine-script-docs/profiling-and-optimization#pine-profiler):
...
The
[profiled results](/pine-script-docs/profiling-and-optimization#interpreting-profiled-results) over 20,735 script executions show the call to
...
using
[different arguments](/pine-script-docs/profiling-and-optimization#profiling-across-configurations) to gauge how its runtime scales.

Here, we changed the `length` argument in each function call from 20 to
200 and
[profiled the script](/pine-script-docs/profiling-and-optimization#profiling-ascript) again to observe the changes in performance. The time spent
...
Note that:
    ...
    -   In many scenarios, a script\'s runtime can benefit from using
        ...
        *high-impact code* and the specific built-ins used. In any case,
        one should always
        [profile their scripts](/pine-script-docs/profiling-and-optimization#profiling-ascript), preferably
        [several times](/pine-script-docs/profiling-and-optimization#repetitive-profiling), when exploring optimized solutions.
    -   The calculations performed by the functions in this example also
        ...
        [different datasets](/pine-script-docs/profiling-and-optimization#profiling-across-configurations) as well.

### Reducing repetition 

The Pine Script™ compiler can automatically simplify some types of
[repetitive code](/pine-script-docs/profiling-and-optimization#insignificant-unused-and-redundant-code) without a programmer\'s intervention. However, this
...
The
[profiled results](/pine-script-docs/profiling-and-optimization#interpreting-profiled-results) for this script show that it spent about 2.5 seconds
...
[function's local code](/pine-script-docs/profiling-and-optimization#user-defined-function-calls) reflect the *combined* time and executions from each
separate call:
...
[profiled results](/pine-script-docs/profiling-and-optimization#interpreting-profiled-results) show a significant improvement in performance, as the script
...
Note that:
    ...
    -   Since this script only calls `valuesAbove()` once, the
        ...
        specific call. See
        [this section](/pine-script-docs/profiling-and-optimization#user-defined-function-calls) to learn more about interpreting profiled function
...
separate calls to
[request.security()](https://www.tradingview.com/pine-script-reference/v5/#fun_request.security).
It then [plots](/pine-script-docs/concepts/plots) all nine
...
The results from
[profiling the script](/pine-script-docs/profiling-and-optimization#profiling-ascript) show that it took the script 340.8 milliseconds to complete
...
As we see below, the
[profiled results](/pine-script-docs/profiling-and-optimization#interpreting-profiled-results) from running this version of the script show that it took
...
Note that:
    ...
    -   The computational resources available to a script **fluctuate**
        ...
        script
        [multiple times](/pine-script-docs/profiling-and-optimization#repetitive-profiling) to help solidify performance conclusions.
...
[plots](/pine-script-docs/concepts/plots). While these types
...
[boxes](/pine-script-docs/concepts/lines-and-boxes#boxes) to using
...
the [boxes](/pine-script-docs/concepts/lines-and-boxes#boxes) in
...
[boxes](/pine-script-docs/concepts/lines-and-boxes#boxes) in the
...
The results from
[profiling this script](/pine-script-docs/profiling-and-optimization#profiling-ascript) show that line 24, which contains the
[box.new()](https://www.tradingview.com/pine-script-reference/v5/#fun_box.new)
call, is the *heaviest* line in the
[code block](/pine-script-docs/profiling-and-optimization#code-block-results) that executes on each bar, with a runtime close to
...
Note that:
    ...
    -   This script updates its drawings over *all bars* in the chart\'s
        ...
        [next section](/pine-script-docs/profiling-and-optimization#reducing-drawing-updates) to learn more.
...
After
[profiling](/pine-script-docs/profiling-and-optimization#profiling-ascript) the script, we see that the code with the highest impact on
performance is the
[for](https://www.tradingview.com/pine-script-reference/v5/#kw_for) loop
that starts on line 20, i.e., the
[code block](/pine-script-docs/profiling-and-optimization#code-block-results) that updates the table\'s cells:
...
[table's](/pine-script-docs/concepts/tables) **final**
...
After
[profiling](/pine-script-docs/profiling-and-optimization#profiling-ascript) the script\'s performance over our chart\'s data, we see
...
[block](/pine-script-docs/profiling-and-optimization#code-block-results) that starts on line 17:
...
[another configuration](/pine-script-docs/profiling-and-optimization#profiling-across-configurations) requiring heavier looping. Here, we set the value to 2500.
...
[profiled results](/pine-script-docs/profiling-and-optimization#interpreting-profiled-results) show that our modified script with a high `lengthInput`
...
Note that:
    ...
    -   Another, more *advanced* way one can further enhance this
        ...
        *single-row*
        ...
        [array](https://www.tradingview.com/pine-script-reference/v5/#type_array)
        as a
        [queue](/pine-script-docs/language/arrays#using-an-array-as-aqueue) to hold recent
        [close](https://www.tradingview.com/pine-script-reference/v5/#var_close)
...
-   Replacing a loop with optimized
    [built-ins](/pine-script-docs/profiling-and-optimization#using-built-ins) where possible
...
[profiled results](/pine-script-docs/profiling-and-optimization#interpreting-profiled-results) with the default settings, we see that it took about 64
...
The
[profiled results](/pine-script-docs/profiling-and-optimization#interpreting-profiled-results) for the script with the default `lengthInput` of 20 show a
...
previous
[configuration](/pine-script-docs/profiling-and-optimization#profiling-across-configurations). In other words, while our original function\'s runtime
...
[next section](/pine-script-docs/profiling-and-optimization#optimizing-loops) for more information.
:::
...
[eliminate](/pine-script-docs/profiling-and-optimization#eliminating-loops), there are [several
...
ways to
[eliminate](/pine-script-docs/profiling-and-optimization#eliminating-loops) it first. If **no solution** exists that makes the loop
...
After
[profiling the script](/pine-script-docs/profiling-and-optimization#profiling-ascript), we see it took about two seconds to execute 21,778 times.
...
[profiled results](/pine-script-docs/profiling-and-optimization#interpreting-profiled-results) from this script run show that it took only 0.6 seconds to
...
[prices` array and `plots](/pine-script-docs/concepts/plots) the
...
As we see below, the
[profiled results](/pine-script-docs/profiling-and-optimization#interpreting-profiled-results) for this script after 20,187 executions show it completed
...
[profiled results](/pine-script-docs/profiling-and-optimization#interpreting-profiled-results), moving the *loop-invariant* calculations outside the loop
...
[polyline](/pine-script-docs/concepts/lines-and-boxes#polylines)
...
drawing. It also [plots](/pine-script-docs/concepts/plots) the
...
As we see from the
[profiled results](/pine-script-docs/profiling-and-optimization#interpreting-profiled-results) after running the script across 20,320 bars, the number of
...
[profiled results](/pine-script-docs/profiling-and-optimization#interpreting-profiled-results) below, the number of global code executions now aligns with
...
Since the
[Pine Profiler](/pine-script-docs/profiling-and-optimization#pine-profiler) must perform *extra calculations* to collect performance
data, as explained in
[this section](/pine-script-docs/profiling-and-optimization#alook-into-the-profilers-inner-workings), the time it takes to execute a script **increases** while
profiling.
...
using the
[Profiler](/pine-script-docs/profiling-and-optimization#pine-profiler) on it may cause its runtime to *exceed* the limit. Such a
case indicates that the script likely needs
[optimization](/pine-script-docs/profiling-and-optimization#optimization), but it can be challenging to know where to start without
being able to
[profile the code](/pine-script-docs/profiling-and-optimization#profiling-ascript). The most effective workaround in this scenario is reducing
...
enabling the
[Profiler](/pine-script-docs/profiling-and-optimization#pine-profiler), the script raises a runtime error stating that it exceeded
...
[Profiler](/pine-script-docs/profiling-and-optimization#pine-profiler) is active. We can try limiting the number of historical
...
[Profiler](/pine-script-docs/profiling-and-optimization#pine-profiler), experiment with different ways to limit its executions
