...
As is explained in the :ref:`Type system <PageTypeSystem>` page, *qualifiers* and *types* play a critical role in determining the type of results that expressions yield.
This, in turn, has an impact on how and with what functions you will be allowed to use those results. 
Expressions always return a value with the strongest qualifier used in the expression, e.g., if you multiply an "input int" with a "series int", 
...
Our problem is that the Reference Manual entry for `ta.ema() <https://www.tradingview.com/pine-script-reference/v5/#fun_ta{dot}ema>`__ tells us that its 
``length`` parameter requires a "simple" value, which is a weaker qualifier than "series", so a "series int" value is not allowed.
...
If both operands are numbers but at least one of these is of `float <https://www.tradingview.com/pine-script-reference/v5/#type_float>`__ type, 
the result will also be a `float <https://www.tradingview.com/pine-script-reference/v5/#type_float>`__. 
If both operands are of `int <https://www.tradingview.com/pine-script-reference/v5/#type_int>`__ type, 
the result will also be an `int <https://www.tradingview.com/pine-script-reference/v5/#type_int>`__.
...
You will find more information on how `var <https://www.tradingview.com/pine-script-reference/v5/#kw_var>`__ works in 
...
- We declare ``pHi`` with this code: ``var float pHi = na``. The `var <https://www.tradingview.com/pine-script-reference/v5/#kw_var>`__ 
...
- While the variable declaration will only be executed on the first bar because it uses `var <https://www.tradingview.com/pine-script-reference/v5/#kw_var>`__, 
