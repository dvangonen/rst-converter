...
sidebar-title: Background coloring with bgcolor
page-title: Annotations / Background coloring with bgcolor
...
the script is running in `overlay=true` mode, then it will color the
chart\'s background. The color used in `bgcolor` can be calculated in an
expression and an optional `transp` parameter (0\--100, default 90)
allows control over the transparency level.

Here is a script that colors the background of trading sessions (try it
on EURUSD, at *30 minutes* resolution):
...
![image](@assets/annotations/Background_coloring_bgcolor_1.png)
