...
sidebar-title: study annotation
page-title: Annotations / study annotation

---
# study annotation

Every *study* (also known as an *indicator*)[^1] script must contain one
call of the
...
`overlay` is a logical argument. If it is true then the study will be
...
When `format.volume` is used, the format is equivalent to `precision=0`
used in earlier versions of Pine, where \"5183\" becomes \"5.183K\".
...
[^1]: Pine also has a
    ...
    `backtesting strategy </essential/Strategies>` rather than a study (or indicator).
