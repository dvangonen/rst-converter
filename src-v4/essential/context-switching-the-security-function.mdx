...
sidebar-title: Context switching and the security function
page-title: Essential / Context switching and the security function
...
Let\'s assume the following script is running on an IBM chart at *1
minute*. It will display the *close* price of the IBM symbol, but at *15
minutes* resolution.
...
![image](@assets/essential/Chart_security_1.png)
...
stored in the
[syminfo.ticker](https://www.tradingview.com/pine-script-reference/v4/#var_syminfo%7Bdot%7Dticker)
and
[syminfo.tickerid](https://www.tradingview.com/pine-script-reference/v4/#var_syminfo%7Bdot%7Dtickerid)
...
indicated by the literal `"1"`. It is possible to request any[^1] number
of minutes: `"5"`, `"10"`, `"21"`, etc. *Hourly* resolution is also set
by minutes[^2]. For example, the following lines signify one hour, two
...
stored in the
[timeframe.period](https://www.tradingview.com/pine-script-reference/v4/#var_timeframe%7Bdot%7Dperiod)
built-in variable.
...
[barmerge.gaps_off](https://www.tradingview.com/pine-script-reference/v4/#var_barmerge%7Bdot%7Dgaps_off),
data is merged continuously (without `na` gaps). All gaps, if any, are
filled with the previous nearest non-`na` value. If
[barmerge.gaps_on](https://www.tradingview.com/pine-script-reference/v4/#var_barmerge%7Bdot%7Dgaps_on)
...
[barmerge.lookahead_off](https://www.tradingview.com/pine-script-reference/v4/#var_barmerge%7Bdot%7Dlookahead_off)
and
[barmerge.lookahead_on](https://www.tradingview.com/pine-script-reference/v4/#var_barmerge%7Bdot%7Dlookahead_on)
...
function. The green line based on historical data is displayed at the
price level of an hourly *low* right after a new hourly bar is created
(dotted blue vertical lines).

The red line is a *low* price of an hourly bar that is requested with
*lookahead off*. In this case the requested *low* price of an hourly
historical bar will be given only on the last minute bar of the
requested hour, when an hourly bar\'s *low* won\'t return future data.
...
![image](@assets/essential/SecurityLowerTF_LookaheadOnOff.png)
...
[^1]: Actually the highest supported minute resolution is \"1440\"
    (which is the number of minutes in 24 hours).

[^2]: Requesting data of `"1h"` or `"1H"` resolution would result in an
    error. Use `"60"` instead.
