...
sidebar-title: Expressions, declarations and statements
page-title: Language / Expressions, declarations and statements
...
produce a result (exceptions are the functions `study`, `fill`,
`strategy.entry`, etc., which produce side effects and will be covered
later).
...
Variables in Pine are declared with the special symbol `=` and an
optional `var` keyword in one of the following ways:
...
`<identifier>` is the name of the declared variable, see
`Identifiers`.

`<type>` can be one of the predefined keywords: `float`, `int`, `bool`,
`color`, `string`, `line` or `label`. However, in most cases, an
explicit type declaration is redundant because type is automatically
inferred from the `<expression>` on the right of the `=` at compile
...
of the `baseLine0` variable because `na` is a generic value of no
particular type. The declaration of the `baseLine1` variable is correct
because its `float` type is declared explicitly. The declaration of the
`baseLine2` variable is also correct because its type can be derived
from the expression `float(na)`, which is an explicit cast of `na` value
to `float` type. The declarations of `baseLine1` and `baseLine2` are
equivalent.

The `var` keyword is a special modifier that instructs the compiler to
...
![image](@assets/language/GreenBarsCount.png)
...
the `count` variable is initialized with an expensive function call
instead of `0`.
...
operator `:=` must be used to give a new value to a variable. A variable
must be declared before you can assign a value to it (see declaration of
variables `above<variable_declaration>`).

The type of a variable is identified at declaration time. From then on,
a variable can be given a value of expression only if both the
expression and the variable belong to the same type, otherwise a
...
An `if` statement defines a block of statements to be executed when the
`if`\'s conditional expression evaluates to `true`, and optionally, an
...
-   `var_declarationX` \-\-- this variable is assigned the value of the
    `if` statement as a whole.
-   `condition` \-\-- if the `condition` expression is true, the logic
    from the *then* block immediately following the `if` first line
...
The type of the returning value of the `if` statement is determined by
the type of `return_expression_then` and `return_expression_else`. Their
...
`if` statement (`var_declarationX =` can be omited). This form is used
when you need the side effect of the expression, for example in
`strategy trading</essential/Strategies>`:
...
The `for` statement allows to execute a number of instructions
repeatedly:
...
-   `to` \-\-- end value of the counter. When the counter becomes
    greater than `to` (or less than `to` in the case where `from > to`)
    the loop is stopped.
-   `step` \-\-- loop step. Optional. Default is 1. If `from` is greater
    than `to`, the loop step will automatically change direction; no
    need to use a negative step.
-   `var_decl0`, \... `var_declN`, `return_expression` \-\-- body of the
    loop. It must be indented by 4 spaces[^2].
-   `return_expression` \-\-- returning value. When a loop is finished
    or broken, the returning value is assigned to `var_declarationX`.
-   `continue` \-\-- a keyword. Can only be used in loops. It jumps to
    the loop\'s next iteration.
-   `break` \-\-- a keyword. Can be used only in loops. It exits the
    loop.
...
Variable `sum` is a
`mutable variable <variable_assignment>`
so a new value can be given to it by the operator `:=` in the loop\'s
...
Note that some built-in functions may behave unexpectedly in for loop.
...
While you may expect that `sum` will contain
`rma(close, 1) + rma(close, 2)`, this is not so. It will contain
`rma(close, 1) + rma(close, 1)` because once `rma` is initialized with
...
-   `valuewhen(condition, source, occurrence)`: `occurrence` is
    stateful.
-   `rsi(x, y)`: when `y` is of type integer and behaves like a length,
    `y` is stateful.

**Footnotes**

[^1]: TradingView\'s *Pine Editor* automatically replaces **Tab** with 4
    spaces.

[^2]: TradingView\'s *Pine Editor* automatically replaces **Tab** with 4
    spaces.
