...
Our `Style guide <PageStyleGuide>`{.interpreted-text role="ref"}
...
`arithmetic <PageOperators_ArithmeticOperators>`{.interpreted-text
role="ref"}, `comparison <ComparisonOperators>`{.interpreted-text
role="ref"},
`logical <PageOperators_LogicalOperators>`{.interpreted-text role="ref"}
or `ternary <PageOperators_TernaryOperator>`{.interpreted-text
role="ref"} operators to form an expression to be assigned to the
...
`type qualifiers <PageTypeSystem_Qualifiers>`{.interpreted-text
...
![image](images/Inputs-InputTypes-01.png){height="50.0%"}
...
`Color selection through script settings <PageColors_ColorSelectionThroughScriptSettings>`{.interpreted-text
role="ref"} section of the \"Colors\" page, the color selections that
...
`sessions <PageSessions>`{.interpreted-text role="ref"} for more
information.
...
`Time <PageTime>`{.interpreted-text role="ref"} page for more
