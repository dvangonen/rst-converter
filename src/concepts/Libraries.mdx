...
`Type system <PageTypeSystem>`. For more
information on user-defined functions and scopes, see the
`User-defined functions <PageUserDefinedFunctions>` page.
...
-   The `// @description`, `// @function`, `// @param` and `// @returns`
    `compiler annotations <PageScriptStructure_CompilerAnnotations>` are optional but we highly recommend you use them. They
...
You can export
`user-defined types (UDTs) <PageTypeSystem_UserDefinedTypes>` from libraries, and library functions can return
`objects <PageObjects>`.
...
-   A default description is built from the
    `compiler annotations <PageScriptStructure_CompilerAnnotations>` we used in our library. We will publish the library
...
![image](@assets/concepts/Libraries-UsingALibrary-1.png)
