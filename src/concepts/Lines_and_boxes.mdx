...
and
[line.delete()](https://www.tradingview.com/pine-script-reference/v5/#fun_line.delete)
built-ins as functions or `methods <PageMethods>`{.interpreted-text
role="ref"}.
...
these
`chart points <PageTypeSystem_Types_ChartPoints>`{.interpreted-text
role="ref"} to determine the line\'s coordinates. Whether it uses the
...
:   Specifies the line\'s style, which can be any of the options listed
    in this page\'s
    `Line styles <PageLinesAndBoxes_Lines_LineStyles>`{.interpreted-text
    role="ref"} section. The default value is
...
[line.new()](https://www.tradingview.com/pine-script-reference/v5/#fun_line.new)
within a `for <PageLoops_For>`{.interpreted-text role="ref"} loop to
...
and
[line.set_color()](https://www.tradingview.com/pine-script-reference/v5/#fun_line.set_color)
as `methods <PageMethods>`{.interpreted-text role="ref"} to update the
line\'s properties:
...
:   -   Each line drawing in this example uses the
        ...
        style. See the
        `Line styles <PageLinesAndBoxes_Lines_LineStyles>`{.interpreted-text
        role="ref"} section below for an overview of all available style
        settings.
...
:   -   *Polylines* can also use any of these variables as their
        ...
        `Creating polylines <PageLinesAndBoxes_Polylines_CreatingPolylines>`{.interpreted-text
        role="ref"} section of this page.
...
and
[line.get_x1()](https://www.tradingview.com/pine-script-reference/v5/#fun_line.get_x1)
getters as `methods <PageMethods>`{.interpreted-text role="ref"} to
...
second point using
[line.get_price()](https://www.tradingview.com/pine-script-reference/v5/#fun_line.get_price)
and `plots <PagePlots>`{.interpreted-text role="ref"} them on the chart:
...
`modify <PageLinesAndBoxes_Boxes_ModifyingLines>`{.interpreted-text
role="ref"} its properties. As we see below, altering the `copiedLine`
...
`uses as a queue <PageArrays_InsertingAndRemovingArrayElements_UsingAnArrayAsAQueue>`{.interpreted-text
role="ref"} to only display the last `numberOfLines` on the chart. When
the
[size](https://www.tradingview.com/pine-script-reference/v5/#fun_array.size)
of the
[array](https://www.tradingview.com/pine-script-reference/v5/#type_array)
...
[linefill](https://www.tradingview.com/pine-script-reference/v5/#type_linefill)
type, see `this <PageFills_LineFills>`{.interpreted-text role="ref"}
section of the `Fills <PageFills>`{.interpreted-text role="ref"} page.
...
As with `lines <PageLinesAndBoxes_Lines>`{.interpreted-text role="ref"},
...
`chart points <PageTypeSystem_Types_ChartPoints>`{.interpreted-text
role="ref"} to set the coordinates of the box\'s corners. Whether it
...
:   Specifies the style of the borders, which can be any of the options
    in the
    `Box styles <PageLinesAndBoxes_Boxes_BoxStyles>`{.interpreted-text
    role="ref"} section of this page.
...
:   -   The `bottomRight` point\'s `index` field is one bar greater than
        ...
        page\'s
        `Creating lines <PageLinesAndBoxes_Lines_CreatingLines>`{.interpreted-text
        role="ref"} section.
    -   Similar to `lines <PageLinesAndBoxes_Lines>`{.interpreted-text
        role="ref"}, if the `topLeft` and `bottomRight` contained
...
Note that, unlike `lines <PageLinesAndBoxes_Lines>`{.interpreted-text
role="ref"}, the `box.*` namespace does not contain a setter function to
modify a box\'s `xloc`. Users must
`create <PageLinesAndBoxes_Boxes_CreatingBoxes>`{.interpreted-text
role="ref"} a new box with the desired `xloc` setting for such cases.
...
`modify <PageLinesAndBoxes_Boxes_ModifyingBoxes>`{.interpreted-text
role="ref"} its properties. As shown on the chart below, these changes
...
values. The script
`creates <PageLinesAndBoxes_Boxes_CreatingBoxes>`{.interpreted-text
role="ref"} a new box ID and stores it in a `boxes` array once every
...
[volume](https://www.tradingview.com/pine-script-reference/v5/#var_volume)
over each period by
`modifying <PageLinesAndBoxes_Boxes_ModifyingBoxes>`{.interpreted-text
role="ref"} the `top` of the
[last](https://www.tradingview.com/pine-script-reference/v5/#fun_array.last)
box in the `boxes` array. The script then uses
`for loops <PageLoops_For>`{.interpreted-text role="ref"} to find the
...
Unlike `lines <PageLinesAndBoxes_Lines>`{.interpreted-text role="ref"}
or `boxes <PageLinesAndBoxes_Boxes>`{.interpreted-text role="ref"},
...
:   Accepts an
    ...
    `chart point <PageTypeSystem_Types_ChartPoints>`{.interpreted-text
    role="ref"} for its x-coordinates depends on the `xloc` value in the
    function call.
...
:   Specifies the style of the polyline, which can be any of the
    ...
    `Line styles <PageLinesAndBoxes_Lines_LineStyles>`{.interpreted-text
    role="ref"} section of this page. The default is
    [line.style_solid](https://www.tradingview.com/pine-script-reference/v5/#var_line.style_solid).
...
:   -   This script uses only *one* polyline to connect each
    ...
    -   While one can achieve a similar effect using
        `lines <PageLinesAndBoxes_Lines>`{.interpreted-text role="ref"},
...
with `lines <PageLinesAndBoxes_Lines>`{.interpreted-text role="ref"} or
`boxes <PageLinesAndBoxes_Boxes>`{.interpreted-text role="ref"}. When
...
`chart points <PageTypeSystem_Types_ChartPoints>`{.interpreted-text
role="ref"} in our example\'s `points` array to demonstrate this
...
`lines <PageLinesAndBoxes_Lines>`{.interpreted-text role="ref"}, whereas
...
values, then uses a `for loop <PageLoops_For>`{.interpreted-text
role="ref"} to push `numberOfSides` new
`chart points <PageTypeSystem_Types_ChartPoints>`{.interpreted-text
role="ref"} into the
[array](https://www.tradingview.com/pine-script-reference/v5/#type_array)
...
which it
`uses as a queue <PageArrays_InsertingAndRemovingArrayElements_UsingAnArrayAsAQueue>`{.interpreted-text
role="ref"} to manage the number of drawings it displays.

When the `newSpiral` condition occurs, the script creates a `points`
array and adds
`chart points <PageTypeSystem_Types_ChartPoints>`{.interpreted-text
role="ref"} within a `for loop <PageLoops_For>`{.interpreted-text
role="ref"}. On each loop iteration, it calls the `spiralPoint()`
`user-defined function <PageUserDefinedFunctions>`{.interpreted-text
role="ref"} to create a new
[chart.point](https://www.tradingview.com/pine-script-reference/v5/#type_chart.point)
...
:   -   We declared a `MAX_POLYLINES_COUNT` global variable with a
    ...
    -   As with our \"N-sided polygons\" example in the
        `previous section <PageLinesAndBoxes_Polylines_CreatingPolylines_ClosedShapes>`{.interpreted-text
        role="ref"}, we round the calculation of x-coordinates to the
...
`collections <PageTypeSystem_Types_Collections>`{.interpreted-text
role="ref"} by *deleting* the existing polylines and assigning *new
...
`chart points <PageTypeSystem_Types_ChartPoints>`{.interpreted-text
role="ref"} that track the period\'s developing OHLC values. As new
...
`deletes <PageLinesAndBoxes_Polylines_DeletingPolylines>`{.interpreted-text
role="ref"} the polyline referenced by the `currentDrawing` before
`creating a new one <PageLinesAndBoxes_Polylines_CreatingPolylines>`{.interpreted-text
role="ref"}, resulting in a dynamic drawing that changes over the
developing period:
...
`Lines <PageLinesAndBoxes_Lines>`{.interpreted-text role="ref"},
`boxes <PageLinesAndBoxes_Boxes>`{.interpreted-text role="ref"}, and
`polylines <PageLinesAndBoxes_Polylines>`{.interpreted-text role="ref"}
...
Pine Script™\'s `Execution model <PageExecutionModel>`{.interpreted-text
role="ref"}.
...
Scripts cannot use `lines <PageLinesAndBoxes_Lines>`{.interpreted-text
role="ref"}, `boxes <PageLinesAndBoxes_Boxes>`{.interpreted-text
role="ref"}, or
`polylines <PageLinesAndBoxes_Polylines>`{.interpreted-text role="ref"}
...
`conditional structure <PageConditionalStructures>`{.interpreted-text
role="ref"}:
