...
we cover in the
`Conditional plots <PagePlots_ConditionalPlots>` section of this page.
...
`color bars <PageBarColoring>` in the
chart area.
...
The
`Conditional coloring <PageColors_ConditionalColoring>` section of the page on colors discusses color control for
plots. We\'ll look here at a few examples.

The value of the `color` parameter in
[plot()](https://www.tradingview.com/pine-script-reference/v5/#fun_plot)
can be a constant, such as one of the built-in
`constant colors <PageColors_ConstantColors>` or a
`color literal <PageTypeSystem_Types_Color>`. In Pine Script™, the qualified type of such colors is
called **\"const color\"** (see the
`Type system <PageTypeSystem>` page). They
...
level transitions. Using `lines <PageLinesAndBoxes>` is one alternative, but you can also use
[plot()](https://www.tradingview.com/pine-script-reference/v5/#fun_plot)
like this:
...
the page on `Colors <PageColors>` for more
information on the
