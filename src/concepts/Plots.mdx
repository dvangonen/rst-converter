...
`Fills <PageFills>`{.interpreted-text role="ref"}.
...
we cover in the
`Conditional plots <PagePlots_ConditionalPlots>`{.interpreted-text
...
`color bars <PageBarColoring>`{.interpreted-text role="ref"} in the
chart area.
...
The
`Conditional coloring <PageColors_ConditionalColoring>`{.interpreted-text
role="ref"} section of the page on colors discusses color control for
plots. We\'ll look here at a few examples.

The value of the `color` parameter in
[plot()](https://www.tradingview.com/pine-script-reference/v5/#fun_plot)
can be a constant, such as one of the built-in
`constant colors <PageColors_ConstantColors>`{.interpreted-text
role="ref"} or a
`color literal <PageTypeSystem_Types_Color>`{.interpreted-text
role="ref"}. In Pine Script™, the qualified type of such colors is
called **\"const color\"** (see the
`Type system <PageTypeSystem>`{.interpreted-text role="ref"} page). They
...
level transitions. Using `lines <PageLinesAndBoxes>`{.interpreted-text
role="ref"} is one alternative, but you can also use
[plot()](https://www.tradingview.com/pine-script-reference/v5/#fun_plot)
like this:
...
`Levels <PageLevels>`{.interpreted-text role="ref"}).
[hline()](https://www.tradingview.com/pine-script-reference/v5/#fun_hline)
...
the page on `Colors <PageColors>`{.interpreted-text role="ref"} for more
information on the
