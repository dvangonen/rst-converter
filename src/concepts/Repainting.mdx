...
sidebar-title: Repainting
page-title: Concepts / Repainting
...
-   **Widespread but often acceptable**: A script may use values that
    ...
    `Historical and realtime behavior <PageOtherTimeframesAndData_HistoricalAndRealtimeBehavior>` section of the
    `Other timeframes and data <PageOtherTimeframesAndData>` page. As with the unconfirmed chart bar in the chart\'s
    timeframe,
    [request.security()](https://www.tradingview.com/pine-script-reference/v5/#fun_request.security)
...
-   **Unacceptable**: Scripts that leak future information into the
    ...
    `non-standard charts <PageNonStandardChartsData>`, and scripts using realtime intrabars to generate alerts
...
`publish <PagePublishing>` scripts, ensure
...
:::warning[Notice!]
We will not discuss the perils of using strategies on non-standard
charts, as this problem is not related to repainting. See the
[Backtesting on Non-Standard Charts:
Caution!](https://www.tradingview.com/script/q9laJNG9-Backtesting-on-Non-Standard-Charts-Caution-PineCoders-FAQ/)
...
`publish <PagePublishing>` your script,
...
-   `Historical vs realtime calculations <PageRepainting_HistoricalVsRealtimeCalculations>`
-   `Plotting in the past <PageRepainting_PlottingInThePast>`
-   `Dataset variations <PageRepainting_DatasetVariations>`
...
[low](https://www.tradingview.com/pine-script-reference/v5/#var_low) and
[close](https://www.tradingview.com/pine-script-reference/v5/#var_close)
values (OHLC).
...
![image](@assets/concepts/Repainting-01.png)
...
`Historical and realtime behavior <PageOtherTimeframesAndData_HistoricalAndRealtimeBehavior>` section of the
`Other timeframes and data <PageOtherTimeframesAndData>` page for a detailed explanation.
...
[request.security()](https://www.tradingview.com/pine-script-reference/v5/#fun_request.security)
call, as explained
`here <PageOtherTimeframesAndData_HistoricalAndRealtimeBehavior_AvoidingRepainting_HigherTimeframeData>`.
...
bars (the ones with an orange background), the `repaintingClose`
contains values that fluctuate without confirmation from the
`higherTimeframe`, meaning it will *repaint* when the script restarts
its execution. The `nonRepaintingClose`, on the other hand, behaves the
same on realtime and historical bars, i.e., it only changes its value
when new, confirmed data is available:

![image](@assets/concepts/Repainting-Repainting-request-security-calls-1.png)
...
Note that:
    ...
    -   We used the
        ...
        on the `higherTimeframe`.
    -   This script produces a [runtime
        error](https://www.tradingview.com/pine-script-reference/v5/#fun_runtime.error)
        if the `higherTimeframe` is lower than the chart\'s timeframe.
    -   On historical bars, the `repaintingClose` has a new value at the
...
Note that:
    ...
    -   The `[1]` offset to the series and the use of
        `lookahead = barmerge.lookahead_on` are interdependent. One
        **cannot** be removed without compromising the integrity of the
        function.
    -   Unlike a plain
        [request.security()](https://www.tradingview.com/pine-script-reference/v5/#fun_request.security)
        call, this wrapper function cannot accept tuple `expression`
        arguments. For multi-element use cases, one can pass a
        `user-defined type <PageTypeSystem_UserDefinedTypes>` whose fields contain the desired elements to
        request.
...
as explained in
`this <PageOtherTimeframesAndData_HistoricalAndRealtimeBehavior_AvoidingRepainting_LowerTimeframeData>` section of the
`Other timeframes and data <PageOtherTimeframesAndData>` page.
...
![image](@assets/concepts/Repainting-FutureLeakWithRequestSecurity-01.png)
...
`this section <PageRepainting_HistoricalVsRealtimeCalculations_RepaintingRequestSecurityCalls>`.

Using lookahead to produce misleading results is not allowed in script
publications, as explained in the
`lookahead <PageOtherTimeframesAndData_CommonCharacteristics_Lookahead>` section of the
`Other timeframes and data <PageOtherTimeframesAndData>` page. Script publications that use this misleading technique
**will be moderated**.
...
Scripts using `bar states <PageBarStates>`
...
[open](https://www.tradingview.com/pine-script-reference/v5/#open).
...
![image](@assets/concepts/Repainting-PlottingInThePast-01.png)
...
on Pine Script™\'s
`execution model <PageExecutionModel>`. If
...
[ta.valuewhen()](https://www.tradingview.com/pine-script-reference/v5/#fun_ta%7Bdot%7Dvaluewhen),
