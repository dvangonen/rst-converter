...
`publish <PagePublishing>`{.interpreted-text role="ref"} your script,
...
-   `Historical vs realtime calculations <PageRepainting_HistoricalVsRealtimeCalculations>`{.interpreted-text
    role="ref"}
-   `Plotting in the past <PageRepainting_PlottingInThePast>`{.interpreted-text
    role="ref"}
-   `Dataset variations <PageRepainting_DatasetVariations>`{.interpreted-text
    role="ref"}
...
`Historical and realtime behavior <PageOtherTimeframesAndData_HistoricalAndRealtimeBehavior>`{.interpreted-text
role="ref"} section of the
`Other timeframes and data <PageOtherTimeframesAndData>`{.interpreted-text
role="ref"} page for a detailed explanation.
...
[request.security()](https://www.tradingview.com/pine-script-reference/v5/#fun_request.security)
call, as explained
`here <PageOtherTimeframesAndData_HistoricalAndRealtimeBehavior_AvoidingRepainting_HigherTimeframeData>`{.interpreted-text
role="ref"}.
...
:   -   The `[1]` offset to the series and the use of
    ...
    -   Unlike a plain
        ...
        `user-defined type <PageTypeSystem_UserDefinedTypes>`{.interpreted-text
        role="ref"} whose fields contain the desired elements to
        request.
...
as explained in
`this <PageOtherTimeframesAndData_HistoricalAndRealtimeBehavior_AvoidingRepainting_LowerTimeframeData>`{.interpreted-text
role="ref"} section of the
`Other timeframes and data <PageOtherTimeframesAndData>`{.interpreted-text
role="ref"} page.
...
`this section <PageRepainting_HistoricalVsRealtimeCalculations_RepaintingRequestSecurityCalls>`{.interpreted-text
role="ref"}.

Using lookahead to produce misleading results is not allowed in script
publications, as explained in the
`lookahead <PageOtherTimeframesAndData_CommonCharacteristics_Lookahead>`{.interpreted-text
role="ref"} section of the
`Other timeframes and data <PageOtherTimeframesAndData>`{.interpreted-text
role="ref"} page. Script publications that use this misleading technique
**will be moderated**.
...
`varip  <PageVariableDeclarations_Varip>`{.interpreted-text role="ref"}
...
Scripts using `bar states <PageBarStates>`{.interpreted-text role="ref"}
...
on Pine Script™\'s
`execution model <PageExecutionModel>`{.interpreted-text role="ref"}. If
