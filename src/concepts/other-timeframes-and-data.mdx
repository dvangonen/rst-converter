...
-   `request.security() <PageOtherTimeframesAndData_RequestSecurity>` retrieves data from another symbol, timeframe, or other
    context.
-   `request.security_lower_tf() <PageOtherTimeframesAndData_RequestSecurityLowerTf>` retrieves *intrabar* data, i.e., data from a timeframe
    lower than the chart timeframe.
-   `request.currency_rate() <PageOtherTimeframesAndData_RequestCurrencyRate>` requests a *daily rate* to convert a value expressed in
    one currency to another.
-   `request.dividends(), request.splits(), and request.earnings() <PageOtherTimeframesAndData_RequestDividendsRequestSplitsAndRequestEarnings>` respectively retrieve information about an issuing
    company\'s dividends, splits, and earnings.
-   `request.quandl() <PageOtherTimeframesAndData_RequestQuandl>` retrieves information from [NASDAQ Data
    Link](https://data.nasdaq.com/) (formerly Quandl).
-   `request.financial() <PageOtherTimeframesAndData_RequestFinancial>` retrieves financial data from
    [FactSet](https://www.factset.com/).
-   `request.economic() <PageOtherTimeframesAndData_RequestEconomic>` retrieves economic and industry data.
-   `request.seed() <PageOtherTimeframesAndData_RequestSeed>` retrieves data from a *user-maintained* GitHub
    repository.
...
`qualified type <PageTypeSystem_Qualifiers>` that each parameter accepts, and these values cannot change
...
`conditional structures <PageConditionalStructures>`, or
functions and methods exported by
`Libraries <PageLibraries>`, but they can
...
`this <PageLimitations_RequestCalls>`
...
`plotting <PagePlots>` the results from
two
[request.security()](https://www.tradingview.com/pine-script-reference/v5/#fun_request.security)
...
Note that:
    ...
    -   When using
        ...
        `Historical and realtime behavior <PageOtherTimeframesAndData_HistoricalAndRealtimeBehavior>` section of this page.
...
`user-defined function <PageUserDefinedFunctions>` to retrieve data for estimating an instrument\'s market
...
Note that:
    ...
    -   Not all issuing companies publish quarterly financial reports.
        ...
        `request.financial() <PageOtherTimeframesAndData_RequestFinancial>` section for more information.
    -   We\'ve used
...
-   The function call requests data from an intrabar timeframe, i.e., a
    ...
    [timeframe.period](https://www.tradingview.com/pine-script-reference/v5/#var_timeframe.period).
    See
    `this section <PageOtherTimeframesAndData_RequestSecurity_Timeframes_LowerTimeframes>` for more information.
...
`publish your script <PagePublishing>` to
...
`plots <PagePlots>` the resulting series
...
Note that:
    ...
    -   The series that uses lookahead and a historical offset
        ...
        *confirmed* value from the higher timeframe. See the
        `Avoiding repainting <PageOtherTimeframesAndData_HistoricalAndRealtimeBehavior_AvoidingRepainting>` section of this page for more information.
...
-   Other timeframes (see our User Manual\'s page on
    `Timeframes <PageTimeframes>` to learn
    about timeframe specifications in Pine Script™)
-   `Custom contexts <PageOtherTimeframesAndData_CustomContexts>`, including alternative sessions, price adjustments,
...
-   The
    ...
    see
    `this <PageChartInformation_SymbolInformation>` section of our
    `Chart information <PageChartInformation>` page.
-   A custom ticker identifier created using `ticker.*()` functions.
    ...
    `non-standard chart <PageNonStandardChartsData>` calculations, alternative sessions, and other contexts.
    See the
    `Custom contexts <PageOtherTimeframesAndData_CustomContexts>` section for more information.

The `timeframe` value specifies the timeframe of the requested data.
This parameter accepts \"string\" values in our
`timeframe specification <PageTimeframes_TimeframeStringSpecifications>` format (e.g., a value of \"1D\" represents the daily
...
`string <PageTypeSystem_Types_String>`,
and `chart.point <PageTypeSystem_Types_ChartPoints>` types. It can also accept
`tuples <PageTypeSystem_Tuples>`,
`collections <PageTypeSystem_Types_Collections>`,
`user-defined types <PageTypeSystem_UserDefinedTypes>`, and the outputs of function and
`method <PageMethods>` calls. For more
details on the data one can retrieve, see the
`Requestable data <PageOtherTimeframesAndData_RequestSecurity_RequestableData>` section below.
...
Note that:
    ...
    -   We\'ve included an offset to the `expression` argument and used
        ...
        `Avoiding repainting <PageOtherTimeframesAndData_HistoricalAndRealtimeBehavior_AvoidingRepainting>` section for more information.
...
script `plots <PagePlots>` the outputs of
...
Note that:
    ...
    -   The script determines the value of the `lowerTimeframe` by
        ...
        `valid timeframe string <PageTimeframes_TimeframeStringSpecifications>` via
    ...
    -   Scripts can retrieve up to 100,000 intrabars from a
        ...
        `this <PageLimitations_RequestCalls_Intrabars>` section of the
        `Limitations <PageLimitations>`
        page.
...
`this section <PageOtherTimeframesAndData_RequestSecurityLowerTf>` to learn more.
:::
...
fundamental type (`int <PageTypeSystem_Types_Int>`, `float <PageTypeSystem_Types_Float>`, `bool <PageTypeSystem_Types_Bool>`, `color <PageTypeSystem_Types_Color>`, or `string <PageTypeSystem_Types_String>`). It can also request the IDs of data structures and
built-in or
`user-defined types <PageTypeSystem_UserDefinedTypes>` that reference fundamental types. The data this function
requests depends on its `expression` parameter, which accepts any of the
following arguments:

-   `Built-in variables and function calls <PageOtherTimeframesAndData_RequestSecurity_RequestableData_BuiltInVariablesAndFunctions>`
-   `Variables calculated by the script <PageOtherTimeframesAndData_RequestSecurity_RequestableData_CalculatedVariables>`
-   `Tuples <PageOtherTimeframesAndData_RequestSecurity_RequestableData_Tuples>`
-   `Calls to user-defined functions <PageOtherTimeframesAndData_RequestSecurity_RequestableData_UserDefinedFunctions>`
-   `Chart points <PageOtherTimeframesAndData_RequestSecurity_RequestableData_ChartPoints>`
-   `Collections <PageOtherTimeframesAndData_RequestSecurity_RequestableData_Collections>`
-   `User-defined types <PageOtherTimeframesAndData_RequestSecurity_RequestableData_UserDefinedTypes>`
...
`Scope count <PageLimitations_ScriptSizeAndMemory_ScopeCount>` section of the
`Limitations <PageLimitations>` page for
more information.
:::
...
function/`method <PageMethods>` call from
...
Note that:
    ...
    -   The script calculates the ribbon\'s higher timeframes by
        ...
        `valid timeframe string <PageTimeframes_TimeframeStringSpecifications>` using
        [timeframe.from_seconds()](https://www.tradingview.com/pine-script-reference/v5/#fun_timeframe.from_seconds).
    -   Instead of calling
        [ta.sma()](https://www.tradingview.com/pine-script-reference/v5/#fun_ta.sma)
        within each
        ...
        `next section <PageOtherTimeframesAndData_RequestSecurity_RequestableData_CalculatedVariables>` for more information.
    -   On realtime bars, this script also tracks *unconfirmed* SMA
        values from each higher timeframe. See the
        `Historical and realtime behavior <PageOtherTimeframesAndData_HistoricalAndRealtimeBehavior>` section to learn more.
...
`plots <PagePlots>` the result on the
chart:
...
Note that:
    ...
    -   The script colors the plot with a
        ...
        `this <PageColors_CalculatedColors_ColorFromGradient>` section of our User Manual\'s page on
        `colors <PageColors>`.

#### Tuples 

`Tuples <PageTypeSystem_Tuples>` in Pine
...
`Requestable data <PageOtherTimeframesAndData_RequestSecurity_RequestableData>` section of this page, excluding other tuples.

:::warning[Notice!]
The combined size of all tuples returned by `request.*()` calls in a
script cannot exceed 127 elements. See
`this <PageLimitations_RequestCalls_TupleElementLimit>` section of the
`Limitations <PageLimitations>` page for
more information.
:::
...
`plots <PagePlots>` the `requestedRank`
...
Note that:
    ...
    -   The
        ...
        `this section <PageTypeSystem_Tuples>` of our User Manual\'s
        `Type system <PageTypeSystem>`
        page.
...
`Requestable data <PageOtherTimeframesAndData_RequestSecurity_RequestableData>` section.
...
`tuple <PageTypeSystem_Tuples>` of custom
band values. The script calls the `weightedBB()` as the `expression`
argument in
[request.security()](https://www.tradingview.com/pine-script-reference/v5/#fun_request.security)
to retrieve a
`tuple <PageOtherTimeframesAndData_RequestSecurity_RequestableData_Tuples>` of band values calculated on the specified `timeframe` and
`plots <PagePlots>` the results on the
chart:
...
Note that:
    ...
    -   We offset the `source` and `weight` arguments in the
        ...
        `this section <PageOtherTimeframesAndData_HistoricalAndRealtimeBehavior_AvoidingRepainting>` to learn more.

#### Chart points 

`Chart points <PageTypeSystem_Types_ChartPoints>` are reference types that represent coordinates on the chart.
...
`chart points <PageTypeSystem_Types_ChartPoints>` from a higher timeframe and uses them to draw
`boxes <PageLinesAndBoxes_Boxes>` on the
...
[request.security()](https://www.tradingview.com/pine-script-reference/v5/#fun_request.security)
to request a
`tuple <PageOtherTimeframesAndData_RequestSecurity_RequestableData_Tuples>` containing the IDs of
`chart points <PageTypeSystem_Types_ChartPoints>` representing the `topLeft` and `bottomRight` from a
`higherTimeframe`.
...
Pine Script™ *collections* (`arrays <PageArrays>`,
and `maps <PageMaps>`) are data structures
that contain an arbitrary number of elements with specified types. The
[request.security()](https://www.tradingview.com/pine-script-reference/v5/#fun_request.security)
function can retrieve the IDs of
`collections <PageTypeSystem_Types_Collections>` whose elements consist of:

-   Fundamental types
-   `Chart points <PageTypeSystem_Types_ChartPoints>`
-   `User-defined types <PageTypeSystem_UserDefinedTypes>` that satisfy the criteria listed in the
    `section below <PageOtherTimeframesAndData_RequestSecurity_RequestableData_UserDefinedTypes>`
...
`maps <PageMaps>` to hold the values used
in the calculations.
...
calculate the `ratio` that it `plots <PagePlots>` in the chart pane:
...
Note that:
    ...
    -   The
        ...
        cannot call `methods <PageMethods>` on a
...
`User-defined types (UDTs) <PageTypeSystem_UserDefinedTypes>` are *composite types* containing an arbitrary number of
...
[request.security()](https://www.tradingview.com/pine-script-reference/v5/#fun_request.security)
function can retrieve the IDs of
`objects <PageObjects>` produced by
`UDTs <PageTypeSystem_UserDefinedTypes>`
from other contexts if their fields consist of:

-   Fundamental types
-   `Chart points <PageTypeSystem_Types_ChartPoints>`
-   `Collections <PageTypeSystem_Types_Collections>` that satisfy the criteria listed in the
    `section above <PageOtherTimeframesAndData_RequestSecurity_RequestableData_Collections>`
-   Other `UDTs <PageTypeSystem_UserDefinedTypes>` whose fields consist of any of these types

The following example requests an
`object <PageObjects>` ID using a
...
[request.security()](https://www.tradingview.com/pine-script-reference/v5/#fun_request.security)
to retrieve the ID of an `object <PageObjects>` representing the calculated `info` from the specified
`symbol`.
...
`this <PageLimitations_RequestCalls>`
section of our `Limitations <PageLimitations>` page for more information.

:::warning[Notice!]
Working with
[request.security_lower_tf()](https://www.tradingview.com/pine-script-reference/v5/#fun_request.security_lower_tf)
involves frequent usage of `arrays <PageArrays>` since it always returns
[array](https://www.tradingview.com/pine-script-reference/v5/#type_array)
results. We therefore recommend you familiarize yourself with
`arrays <PageArrays>` to make the most of
...
`Requestable data <PageOtherTimeframesAndData_RequestSecurity_RequestableData>` portion of the
`request.security() <PageOtherTimeframesAndData_RequestSecurity>` section above, which provides foundational information about
...
`collections <PageTypeSystem_Types_Collections>` and mutable variables declared in the script\'s main scope.
...
`Scope count <PageLimitations_ScriptSizeAndMemory_ScopeCount>` section of the
`Limitations <PageLimitations>` page to
learn more.
:::
...
[request.security_lower_tf()](https://www.tradingview.com/pine-script-reference/v5/#fun_request.security_lower_tf)
always returns its results as `arrays <PageArrays>`. The elements in the returned
`arrays <PageArrays>` represent the
`expression` values retrieved from the lower timeframe sorted in
ascending order based on each intrabar\'s timestamp.

The `type template <PageTypeSystem_TypeTemplates>` assigned to the returned
`arrays <PageArrays>` corresponds to the
...
[request.security_lower_tf()](https://www.tradingview.com/pine-script-reference/v5/#fun_request.security_lower_tf),
the result is a tuple of `arrays <PageArrays>` with
`type templates <PageTypeSystem_TypeTemplates>` corresponding to the types within the argument. For example,
...
`arrays <PageArrays>` of intrabar data
with a single
[request.security_lower_tf()](https://www.tradingview.com/pine-script-reference/v5/#fun_request.security_lower_tf)
function call.
...
`lines and boxes <PageLinesAndBoxes>`. The
script calls
[request.security_lower_tf()](https://www.tradingview.com/pine-script-reference/v5/#fun_request.security_lower_tf)
with the `[open, high, low, close]` tuple as its `expression` to
retrieve a tuple of `arrays <PageArrays>`
...
around the `lines <PageLinesAndBoxes_Lines>` to indicate the chart region occupied by intrabar drawings:
...
Note that:
    ...
    -   The `lowerTimeframe` value is the result of calculating the
        ...
        `valid timeframe string <PageTimeframes_TimeframeStringSpecifications>` with
        [timeframe.from_seconds()](https://www.tradingview.com/pine-script-reference/v5/#fun_timeframe.from_seconds).
...
`collections <PageTypeSystem_Types_Collections>` from an intrabar context. However, unlike
[request.security()](https://www.tradingview.com/pine-script-reference/v5/#fun_request.security),
one cannot pass
`collections <PageTypeSystem_Types_Collections>` or calls to functions that return them as the `expression`
argument in a
[request.security_lower_tf()](https://www.tradingview.com/pine-script-reference/v5/#fun_request.security_lower_tf)
call, as `arrays <PageArrays>` cannot
directly reference other
`collections <PageTypeSystem_Types_Collections>`.

Despite these limitations, it is possible to request
`collections <PageTypeSystem_Types_Collections>` from lower timeframes, if needed, with the help of *wrapper*
types.
...
how
`user-defined types <PageTypeSystem_UserDefinedTypes>` and
`collections <PageTypeSystem_Types_Collections>` work in Pine Script™. When possible, we recommend using
*simpler* methods to manage LTF requests, and only using this approach
when *nothing else* will suffice.
:::

To make
`collections <PageTypeSystem_Types_Collections>` requestable with
[request.security_lower_tf()](https://www.tradingview.com/pine-script-reference/v5/#fun_request.security_lower_tf),
we must create a
`UDT <PageTypeSystem_UserDefinedTypes>`
with a field to reference a collection ID. This step is necessary since
`arrays <PageArrays>` cannot reference
other `collections <PageTypeSystem_Types_Collections>` directly but *can* reference UDTs with collection fields:
...
`objects <PageObjects>` of the UDT to the
`expression` parameter in
[request.security_lower_tf()](https://www.tradingview.com/pine-script-reference/v5/#fun_request.security_lower_tf).
...
that returns an `object <PageObjects>` of
the `UDT <PageTypeSystem_UserDefinedTypes>` and call that function within
[request.security_lower_tf()](https://www.tradingview.com/pine-script-reference/v5/#fun_request.security_lower_tf).
...
Note that:
    ...
    -   The `intrabarPrices` variable only references a `Prices` ID if
        ...
        `out of bounds error <PageArrays_ErrorHandling_IndexIsOutOfBounds>`.
    ...
    -   The process used in this example is *not* necessary to achieve
        ...
        `UDTs <PageTypeSystem_UserDefinedTypes>` and pass an `[open, high, low, close]` tuple to the
        `expression` parameter to retrieve a tuple of
        `arrays <PageArrays>` for further
        operations, as explained in the
        `previous section <PageOtherTimeframesAndData_RequestSecurityLowerTf_TuplesOfIntrabarData>`.
...
-   [ticker.heikinashi()](https://www.tradingview.com/pine-script-reference/v5/#fun_ticker.heikinashi),
    ...
    `non-standard chart <PageNonStandardChartsData>` settings.
...
[request.security()](https://www.tradingview.com/pine-script-reference/v5/#fun_request.security)
to retrieve a
`tuple <PageOtherTimeframesAndData_RequestSecurity_RequestableData_Tuples>` of adjusted price values, then plots the result as
...
data that uses
`non-standard chart <PageNonStandardChartsData>` calculations. For example, suppose we want to use
...
prices, the `strategy <PageStrategies>`
...
Pine\'s `Execution model <PageExecutionModel>`.
...
`section below <PageOtherTimeframesAndData_HistoricalAndRealtimeBehavior_AvoidingRepainting>` explains how to achieve such a result and avoid repainting
data requests.
...
`Data feeds <PageOtherTimeframesAndData_DataFeeds>` section of this page. Either case may result in intrabar
...
Note that:
    ...
    -   This code may not show intrabar data on all available chart
        ...
        `this <PageLimitations_RequestCalls>` section of the
        `Limitations <PageLimitations>`
...
`this section <PageOtherTimeframesAndData_HistoricalAndRealtimeBehavior_AvoidingRepainting>`.
...
`Common characteristics <PageOtherTimeframesAndData_CommonCharacteristics>` section at the top of this page.
...
the
`Common characteristics <PageOtherTimeframesAndData_CommonCharacteristics>` section of this page.
...
`plots <PagePlots>` the retrieved time
...
`Financial IDs <PageOtherTimeframesAndData_RequestFinancial_FinancialIDs>` section below for an overview of all accessible metrics and
...
`Financial IDs <PageOtherTimeframesAndData_RequestFinancial_FinancialIDs>` section.

See this page\'s
`Common characteristics <PageOtherTimeframesAndData_CommonCharacteristics>` section for a detailed explanation of this function\'s
...
\"TOTAL_OPER_EXPENSE\"
`financial IDs <PageOtherTimeframesAndData_RequestFinancial_FinancialIDs>` for the
[syminfo.tickerid](https://www.tradingview.com/pine-script-reference/v5/#var_syminfo.tickerid)
...
`user-defined functions <PageUserDefinedFunctions>` that calculate the above financial metrics for the
[syminfo.tickerid](https://www.tradingview.com/pine-script-reference/v5/#var_syminfo.tickerid).
...
chart using `labels <PageTextAndShapes_Labels>`. Hovering over any bar\'s
[label](https://www.tradingview.com/pine-script-reference/v5/#type_label)
...
-   `Income statements <PageOtherTimeframesAndData_RequestFinancial_FinancialIDs_IncomeStatements>`
-   `Balance sheet <PageOtherTimeframesAndData_RequestFinancial_FinancialIDs_BalanceSheet>`
-   `Cash flow <PageOtherTimeframesAndData_RequestFinancial_FinancialIDs_CashFlow>`
-   `Statistics <PageOtherTimeframesAndData_RequestFinancial_FinancialIDs_Statistics>`
...
`Country/region codes <PageOtherTimeframesAndData_RequestEconomic_CountryRegionCodes>` section for a complete list of codes this function supports.
...
The
`Field codes <PageOtherTimeframesAndData_RequestEconomic_FieldCodes>` section covers all accessible metrics and the
...
[request.economic()](https://www.tradingview.com/pine-script-reference/v5/#fun_request.economic),
then `plots <PagePlots>` its value on the
...
`field codes <PageOtherTimeframesAndData_RequestEconomic_FieldCodes>` are accessible to the function.
...
demands. See the `Limitations <PageLimitations>` page\'s section on
`scope count <PageLimitations_ScriptSizeAndMemory_ScopeCount>` limits for more information.
:::
