...
As explained in the
`When the script's scale must be preserved <PageDebugging_WhenTheScriptsScaleMustBePreserved>` section of our page on
`Debugging <PageDebugging>`, the function
...
`lines and boxes <PageLinesAndBoxes>`, or
`tables <PageTables>`. Like them, they are
