...
-   `How do I make an alert available from my script? <PageFaq_Alerts_HowDoIMakeAnAlertAvailableFromMyScript>`
...
-   `If I make changes to my script, will it affect my alert? <PageFaq_Alerts_IfIMakeChangesToMyScriptWillItAffectMyAlert>`
-   `I have a custom script that generates alerts. How do I run it on many symbols? <PageFaq_Alerts_IHaveACustomScriptThatGeneratesAlertsHowDoIRunItOnManySymbols>`
-   `Is it possible to use a string that varies as an argument to the alertcondition() function’s message = parameter? <PageFaq_Alerts_IsItPossibleToUseAStringThatVariesAsAnArgumentToTheAlertconditionFunctionsMessage=Parameter>`
-   `How can I include values that change in my alerts? <PageFaq_Alerts_HowCanIIncludeValuesThatChangeInMyAlerts>`
-   `How can I create JSON messages in my alerts? <PageFaq_Alerts_HowCanICreateJsonMessagesInMyAlerts>`
-   `Can my Pine strategy or indicator place automated orders in markets? <PageFaq_Alerts_CanMyPineStrategyOrIndicatorPlaceAutomatedOrdersInMarkets>`
-   `How can I send alerts from my script to Discord? <PageFaq_Alerts_HowCanISendAlertsFromMyScriptToDiscord>`
-   `How can I send alerts to Telegram? <PageFaq_Alerts_HowCanISendAlertsToTelegram>`
-   `Why did my alerts get stopped? <PageFaq_Alerts_WhyDidMyAlertsGetStopped>`
-   `Why is my alert firing when it's not supposed to? <PageFaq_Alerts_WhyIsMyAlertFiringWhenItsNotSupposedTo>`
-   `How can I run my alert on a timer or delay? <PageFaq_Alerts_HowCanIRunMyAlertOnATimerOrDelay>`
-   `How can I trigger an alert only once when the condition is true the first time? <PageFaq_Alerts_HowCanITriggerAnAlertOnlyOnceWhenTheConditionIsTrueTheFirstTime>`

### How do I make an alert available from my script? 

There are three ways in which users can add
`alerts <PageAlerts>` to their scripts.
...
-   The function cannot be used in
    `local scope <PageFaq_Programming_WhatDoesScopeMean>`.
...
-   Every call to the `alertcondition()` function in a script
    ...
    `plot count limit <PageLimitations_ChartVisuals_PlotLimits>`.
...
-   Strategy *order fill alerts* are superior to using the
    ...
    the likelihood of `repainting <PageRepainting>`, improving the reliability of the alerts.
...
-   The `@strategy_alert_message`
    `compiler annotation <PageScriptStructure_CompilerAnnotations>` allows users to set a default message for order fill
...
`order placement commands <PageStrategies_OrdersAndEntries_OrderPlacementCommands>` to notify about entries and exits. These strings combine
...
-   Each `alert()` call does not contribute toward the
    `plot count limit <PageLimitations_ChartVisuals_PlotLimits>`, as `alertcondition()` does.
...
-   Unlike `alertcondition()`, the `alert()` function has no `condition`
    ...
    `local scope <PageFaq_Programming_WhatDoesScopeMean>` to be executed when the branch logic allows.
...
\"`const <PageTypeSystem_Qualifiers_Const>`\" string determined at compile time. This limitation means
you can\'t use string variables with \"series\"
`qualifiers <PageTypeSystem_Qualifiers>`
...
2.  **Using \"series\" strings in alert():** For more customization, you
    can use a
    \"`series <PageTypeSystem_Qualifiers_Series>`\" string within the
    [alert()](https://www.tradingview.com/pine-script-reference/v5/#fun_alert)
...
`FAQ section <PageFaq_Alerts_HowDoIMakeAnAlertAvailableFromMyScript>` about incorporating alerts into your scripts, as well as the
alert-specific section in the
`User Manual <PageAlerts>`.
...
`strategy alerts <PageFaq_Alerts_HowDoIMakeAnAlertAvailableFromMyScript_UsingOrderFillsInStrategyScripts>` due to the comprehensive list of [strategy-specific
...
`alerts <PageAlerts>`. Pine Script™ can be
used to generate
[webhooks](https://www.tradingview.com/support/solutions/43000529348-about-webhooks/)
...
Sending `alerts <PageAlerts>` from your
...
`how do I make an alert available from my Script <PageFaq_Alerts_HowDoIMakeAnAlertAvailableFromMyScript>` and review the
`alert-specific <PageAlerts>` section of
the User Manual. Additionally, our FAQ provides insights on effectively
`sending JSON messages from your scripts <PageFaq_Alerts_HowCanICreateJsonMessagesInMyAlerts>`.
:::
...
-   If your alerts are firing at different times than expected, it might
    ...
    repainting in the `User Manual <PageRepainting>`.
...
-   Runtime errors in your script can lead to halted alerts. While your
    ...
    -   **Array size limitations:** Arrays in Pine Script are limited to
        `100,000 items <PageArrays_ErrorHandling_ArrayIsTooLarge>`. Exceeding this limit will cause errors. Incorporate
        methods that
        `mitigate the size <PageArrays_InsertingAndRemovingArrayElements_Removing>` of arrays to prevent them from becoming larger than
        the limit.
...
to `repainting <PageRepainting>` in your
...
-   **Alerts firing before bar close:** Most indicators can exhibit some
    ...
    `fluctuating results <PageRepainting_HistoricalVsRealtimeCalculations_FluidDataValues>` in realtime until the bar closes. This occurs because
...
-   **Using calc_on_every_tick in strategies:** When a
    `strategy <PageStrategies>` script is
    ...
    on
    `altering the calculation behavior <PageStrategies_AlteringCalculationBehavior>` of strategies.
-   **Incorrect use of request.security() calls:** Incorrect usage of
    ...
    `repainting higher timeframe calls <PageRepainting_HistoricalVsRealtimeCalculations_RepaintingRequestSecurityCalls>` and
    `other timeframes and data <PageOtherTimeframesAndData>`. To see practical examples that illustrate the correct
...
`repainting <PageRepainting>` in the Pine
...
`alerts <PageAlerts>` in Pine Script™ can
...
`varip variable declaration <PageVariableDeclarations_Varip>` in the User Manual. - Refer to the Pine Script™
`execution model <PageExecutionModel>` for
...
`alert <PageAlerts>` to be triggered only
...
linked to a
`third-party order execution engine <PageFaq_Alerts_CanMyPineStrategyOrIndicatorPlaceAutomatedOrdersInMarkets>`, repeated alerts for the same condition can inadvertently
...
`methods to incorporate alerts <PageFaq_Alerts_HowDoIMakeAnAlertAvailableFromMyScript>` into your scripts. For indicators, there is
[alertcondition()](https://www.tradingview.com/pine-script-reference/v5/#fun_alertcondition)
and
[alert()](https://www.tradingview.com/pine-script-reference/v5/#fun_alert).
...
-   `What are the primary data structures available in Pine Script™? <PageFaq_DataStructures_WhatAreThePrimaryDataStructuresAvailableInPineScript>`
    -   `Tuples <PageFaq_DataStructures_WhatAreThePrimaryDataStructuresAvailableInPineScript_Tuples>`
    -   `Arrays <PageFaq_DataStructures_WhatAreThePrimaryDataStructuresAvailableInPineScript_Arrays>`
    -   `Matrices <PageFaq_DataStructures_WhatAreThePrimaryDataStructuresAvailableInPineScript_Matrices>`
    -   `Objects <PageFaq_DataStructures_WhatAreThePrimaryDataStructuresAvailableInPineScript_Objects>`
    -   `Maps <PageFaq_DataStructures_WhatAreThePrimaryDataStructuresAvailableInPineScript_Maps>`
-   `What is a series, and how does it differ from an array in Pine Script™? <PageFaq_DataStructures_WhatIsASeriesAndHowDoesItDifferFromAnArrayInPineScript>`
    -   `What is a Series in Pine Script™? <PageFaq_DataStructures_WhatIsASeriesAndHowDoesItDifferFromAnArrayInPineScript_WhatIsASeriesInPineScript>`
    -   `What is an Array in Pine Script™? <PageFaq_DataStructures_WhatIsASeriesAndHowDoesItDifferFromAnArrayInPineScript_WhatIsAnArrayInPineScript>`
-   `How do I create and use arrays in Pine Script™? <PageFaq_DataStructures_HowDoICreateAndUseArraysInPineScript>`
-   `What's the difference between an array declared with var or without var? <PageFaq_DataStructures_WhatsTheDifferenceBetweenAnArrayDeclaredWithVarOrWithoutVar>`
-   `What does queue/stack mean? <PageFaq_DataStructures_WhatDoesQueueStackMean>`
-   `Can I use matrices or multidimensional arrays in Pine Script™? <PageFaq_DataStructures_CanIUseMatricesOrMultidimentionalArraysInPineScript>`
-   `How can I perform operations on all elements in an array? <PageFaq_DataStructures_HowCanIPerformOperationsOnAllElementsInAnArray>`
-   `How can I efficiently search for an element in an array? <PageFaq_DataStructures_HowCanIEfficientlySearchForAnElementInAnArray>`
-   `How can I debug arrays? <PageFaq_DataStructures_HowCanIDebugArrays>`
    -   `Plotting <PageFaq_DataStructures_HowCanIDebugArrays_Plotting>`
    -   `Using labels <PageFaq_DataStructures_HowCanIDebugArrays_UsingLabels>`
    -   `Using tables <PageFaq_DataStructures_HowCanIDebugArrays_UsingTables>`
    -   `Pine Logs <PageFaq_DataStructures_HowCanIDebugArrays_PineLogs>`
-   `How can I debug objects? <PageFaq_DataStructures_HowCanIDebugObjects>`
...
A `tuple <PageTypeSystem_Tuples>` in Pine
...
`methods <PageMethods>`, or local blocks.
...
`arrays <PageArrays>` can be created with
...
See the `Arrays <PageArrays>` section in
the User Manual for detailed usage information.

#### Matrices 

`Matrices <PageMatrices>` in Pine Script™
...
`type <PageTypeSystem_Types>`, whether
it\'s a `built-in <PageTypeSystem_Qualifiers>` or `user-defined
...
`transpose <PageMatrices_ManipulatingAMatrix_Transposing>`, then calculates the
...
See the `Matrices <PageMatrices>` section
in the User Manual for detailed usage information.

#### Objects 

`Objects <PageObjects>` in Pine Script™
...
`user-defined types <PageTypeSystem_UserDefinedTypes>` (UDTs) are similar to structs in traditional programming
...
See our User Manual's page on `Objects <PageObjects>` to learn more about working with UDTs.
...
script `loops the map <PageMaps_LoopingThroughAMap>`, logging each key-value pair to display the map\'s contents.

To learn more about working with maps, please refer to the
`Maps <PageMaps>` section in the Pine
Script™ User Manual.
...
`execution model <PageExecutionModel>` is
frequently sufficient for most analytical tasks and far more efficient.
The concept of `time series <PageTimeSeries>` is a fundamental aspect of Pine Script\'s design, deeply
...
`[] history-referencing operator <PageOperators_HistoryReferencingOperator>`, enabling the script to access past values of a series by
...
an `array <PageArrays>` represents a more
...
-   **Mutable:** Arrays can have their size changed dynamically by
    ...
    `insert or remove elements <PageArrays_InsertingAndRemovingArrayElements>`. Any value in an array can also be altered using the
...
Pine Script™ `Arrays <PageArrays>` are
...
`var <PageVariableDeclarations_Var>`
...
`here <PageFaq_DataStructures_WhatsTheDifferenceBetweenAnArrayDeclaredWithVarOrWithoutVar>`.
...
`queues <PageArrays_InsertingAndRemovingArrayElements_UsingAnArrayAsAQueue>` or LIFO
`stacks <PageArrays_InsertingAndRemovingArrayElements_UsingAnArrayAsAStack>`, useful for custom datasets or sliding window calculations.
...
`this FAQ entry <PageFaq_DataStructures_WhatDoesQueueStackMean>`.

**Calculations on arrays**

Due to arrays being non-`time series <PageTimeSeries>` data structures, performing operations across an array\'s
...
`Calculation on arrays <PageArrays_CalculationsOnArrays>` section for more information.
...
-   At the start of each session, we update the array by adding the new
    ...
    `queue <PageArrays_InsertingAndRemovingArrayElements_UsingAnArrayAsAQueue>`, keeps the array\'s size constant while maintaining a
    moving window of the session opens for the last 5 days.
-   Utilize special-purpose array functions to perform
    `calculations on the array <PageArrays_CalculationsOnArrays>`, which yield three key values: the highest open, the
...
-   For a practical example of managing lines within an array, refer to
    ...
    `this FAQ entry <PageFaq_Visuals_HowDoIPlotASupportOrATrendLine>`.
...
`array <PageArrays>` variable in a script
...
each array on each bar. We use a `table <PageTables>` to present and compare both the sizes of these arrays and
...
`declaration modes <PageVariableDeclarations_DeclarationModes>` and their use in
`array declarations <PageArrays_DeclaringArrays_UsingVarAndVaripKeywords>` in the Pine Script™ User Manual.

### What does queue/stack mean? 

In Pine Script™, as in many programming languages,
`arrays <PageArrays>` can be used to
implement data structures such as
`queues <PageArrays_InsertingAndRemovingArrayElements_UsingAnArrayAsAQueue>` and
`stacks <PageArrays_InsertingAndRemovingArrayElements_UsingAnArrayAsAStack>`. These concepts are useful for managing collections of data
...
objective is to draw `lines <PageLinesAndBoxes_Lines>` from pivot points and extend these lines with each new bar
...
`Arrays <PageArrays>` section in the Pine
Script™ User Manual.
...
it provides `matrices <PageMatrices>` and
`user-defined
...
-   **Introduction of matrices:** Pine Script™ now supports
    `matrices <PageMatrices>`, which are
...
-   **Type uniformity:** Similar to
    `arrays <PageArrays>`, all elements
    within a matrix must be of the same
    `type <PageTypeSystem_Types>`, which
    can be a `built-in <PageTypeSystem_Qualifiers>` or a
    `user-defined type <PageTypeSystem_UserDefinedTypes>`. This constraint ensures data consistency and
...
-   **Functions and methods:** Pine Script™ provides a range of
    ...
    `manipulation <PageMatrices_ManipulatingAMatrix>` and
    `calculations <PageMatrices_MatrixCalculations>`, including matrix addition, subtraction, multiplication,
...
-   **Using UDTs for multidimensional structures:** While Pine Script™
    ...
    `user-defined types <PageTypeSystem_UserDefinedTypes>` (UDTs). UDTs allow for the creation of complex data
...
-   **Two-argument form:** Utilizes the format
    ...
    and its index in a `tuple <PageTypeSystem_Tuples>`. This variant is useful for operations where the
...
`queue <PageArrays_InsertingAndRemovingArrayElements_UsingAnArrayAsAQueue>` to monitor the four latest pivot highs and lows, using
`lines <PageLinesAndBoxes_Lines>` to
...
Searching within `arrays <PageArrays>` for
specific contents is a common requirement. While iterating through an
array using `loops <PageLoops>` to check
...
1.  **Using \`array.includes()\`:** The
    ...
    `label <PageTextAndShapes_Labels>`.
...
3.  **Using \`array.binary_search()\`:** For arrays sorted in ascending
    ...
    `sorted <PageArrays_ManipulatingArrays_Sorting>` arrays compared to
    [array.indexof()](https://www.tradingview.com/pine-script-reference/v5/#fun_array.indexof).
...
`arrays <PageArrays>` within Pine Script™,
...
`float <PageTypeSystem_Types_Float>` or
...
`labels <PageTextAndShapes_Labels>` to
...
`functions <PageUserDefinedFunctions>` and
`methods <PageMethods_UserDefinedMethods>`
(refer to the
`Debugging function <PageDebugging_DebuggingFunctions>` section for details). Scripts can also position drawings at
...
`debugging with labels <PageDebugging_Strings_UsingLabels>` enables the complete visualization of all elements in an
array, there is a limitation on the maximum number of labels that a
script can display, as determined by a
`set limit on labels per script <PageLimitations_ChartVisuals_LineBoxPolylineAndLabelLimits>`.

#### Using tables 

In Pine Script™, `tables <PageTables>` are
...
`"series" <PageTypeSystem_Qualifiers_Series>` strings in a clear format that remains unaffected by the
...
`queues <PageArrays_InsertingAndRemovingArrayElements_UsingAnArrayAsAQueue>`, their size is kept manageable for display purposes, while
...
`Pine Logs <PageDebugging_PineLogs>` are
...
`this FAQ entry <PageFaq_DataStructures_WhatDoesQueueStackMean>`, enhancing it by integrating Pine Logs for logging. This
...
`create console messages <PageDebugging_PineLogs_CreatingLogs>` that are time stamped and offer precise information about
...
`stack-based approach <PageArrays_InsertingAndRemovingArrayElements_UsingAnArrayAsAStack>` for tracking lines drawn from pivot points. When a pivot
...
`local scopes <PageDebugging_DebuggingFunctions_LocalDrawingsAndLogs>`,
`functions <PageDebugging_DebuggingFunctions>`, `loops <PageDebugging_DebuggingLoops>`, or other logical constructs. By logging messages to the
...
refer to the
`Error Handling <PageArrays_ErrorHandling>` section specifically dedicated to arrays in the Pine Script™
User Manual.
:::
...
`objects <PageObjects>` into strings for
...
`Debugging <PageDebugging>` section for
more info). A particularly effective debugging method involves the use
of `Pine Logs <PageDebugging_PineLogs>`.
...
`user-defined type <PageTypeSystem_UserDefinedTypes>` (UDT) named `openLine`, which includes fields such as
...
-   `Can I use a variable length in functions? <PageFaq_Functions_CanIUseAVariableLengthInFunctions>`
-   `How can I calculate values depending on variable lengths that reset on a condition? <PageFaq_Functions_HowCanICalculateValuesDependingOnVariableLengthsThatResetOnACondition>`
-   `How can I round a number to x increments? <PageFaq_Functions_HowCanIRoundANumberToXIncrements>`
-   `How can I control the number of decimals used in displaying my script's values? <PageFaq_Functions_HowCanIControlTheNumberOfDecimalsUsedInDisplayingMyScriptsValues>`
-   `How can I control the precision of values used in my calculations? <PageFaq_Functions_HowCanIControlThePrecisionOfValuesUsedInMyCalculations>`
-   `How can I round to ticks? <PageFaq_Functions_HowCanIRoundToTicks>`
-   `How can I abbreviate large values? <PageFaq_Functions_HowCanIAbbreviateLargeValues>`
-   `How can I calculate using pips? <PageFaq_Functions_HowCanICalculateUsingPips>`
-   `How do I calculate averages? <PageFaq_Functions_HowDoICalculateAverages>`
-   `How can I calculate an average only when a certain condition is true? <PageFaq_Functions_HowCanICalculateAnAverageOnlyWhenACertainConditionIsTrue>`
-   `How can I generate a random number? <PageFaq_Functions_HowCanIGenerateARandomNumber>`
-   `How can I evaluate a filter I am planning to use? <PageFaq_Functions_HowCanIEvaluateAFilterIAmPlanningToUse>`
-   `What does nz() do? <PageFaq_Functions_WhatDoesNzDo>`
...
Initially, most `built-in <PageBuiltInFunctions>` functions in the `ta.*()` namespace were limited to
accepting only
\"`simple <PageTypeSystem_Qualifiers_Simple>`\" integer values for their `length` arguments. However,
there\'s been a shift towards supporting
\"`series <PageTypeSystem_Qualifiers_Series>`\" lengths, enabling dynamic adjustment of this parameter
...
-   **User-defined functions:** For cases where built-in functions do
    ...
    library, creating a
    `user-defined function <PageUserDefinedFunctions>` with custom logic is necessary.
...
`Release notes <PageReleaseNotes>`. The
...
3.  **Use a custom function:** If you want more granular control over
    ...
    `user-defined function <PageUserDefinedFunctions>` in the following code example called
...
4.  **Custom datasets:** If you need to average a custom set of values,
    ...
    `Pine User Manual <PageArrays>` on
...
-   Import the
    ...
    `Libraries <PageLibraries>` section of
...
-   `Can I create an indicator that plots like the built-in Volume or Volume Profile indicators? <PageFaq_Indicators_CanICreateAnIndicatorThatplotsLikeTheBuiltInVolumeOrVolumeProfileIndicators>`
-   `Can I use a Pine script with the TradingView screener? <PageFaq_Indicators_CanIUseAPineScriptWithTheTradingviewScreener>`
-   `How can I use one script's output as an input into another? <PageFaq_Indicators_HowCanIUseOneScriptsOutputAsAnInputIntoAnother>`
-   `Can my script place something on the chart when it is running from a pane? <PageFaq_Indicators_CanMyScriptPlaceSomethingOnTheChartWhenItIsRunningFromAPane>`
-   `Is it possible to export indicator data to a file? <PageFaq_Indicators_IsItPossibleToExportIndicatorDataToAFile>`
...
To effectively run a custom script that generates alerts on multiple
symbols, refer to
`this FAQ entry <PageFaq_Alerts_IHaveACustomScriptThatGeneratesAlertsHowDoIRunItOnManySymbols>`, which uses the `request.security()` function to call data
...
`User Manual <PageInputs_InputTypes_SourceInput>`.
...
-   `` How to avoid repainting when using the \`request.security()\` function? <PageFaq_OtherDataAndTimeframes_HowToAvoidRepaintingWhenUsingTheRequestSecurityFunction> ``
    -   `Higher timeframes <PageFaq_OtherDataAndTimeframes_HowToAvoidRepaintingWhenUsingTheRequestSecurityFunction_HigherTimeframes>`
    -   `Lower timeframes <PageFaq_OtherDataAndTimeframes_HowToAvoidRepaintingWhenUsingTheRequestSecurityFunction_LowerTimeframes>`
    -   `Timeframes equal to the chart's timeframe <PageFaq_OtherDataAndTimeframes_HowToAvoidRepaintingWhenUsingTheRequestSecurityFunction_TimeframesEqualToTheChartsTimeframe>`
-   `How can I convert the chart's timeframe into a numeric format? <PageFaq_OtherDataAndTimeframes_HowCanIConvertTheChartsTimeframeIntoANumericFormat>`
...
-   `How do I define a higher interval that is a multiple of the current one? <PageFaq_OtherDataAndTimeframes_HowDoIDefineAHigherIntervalThatIsAMultipleOfTheCurrentOne>`
...
-   `How can I fetch the ATR value from a higher timeframe? <PageFaq_OtherDataAndTimeframes_HowCanIFetchTheAtrValueFromAHigherTimeframe>`
-   `How can I plot a moving average only when the chart’s timeframe is 1D or higher? <PageFaq_OtherDataAndTimeframes_HowCanIPlotAMovingAverageOnlyWhenTheChartsTimeframeIs1dOrHigher>`
-   `How can I plot a moving average calculated using the 1H timeframe on any chart? <PageFaq_OtherDataAndTimeframes_HowCanIPlotAMovingAverageCalculatedUsingThe1hTimeframeOnAnyChart>`
...
`repainting <PageRepainting>` for each of
the 3 scenarios.
...
`Historical and realtime behavior <PageOtherTimeframesAndData_HistoricalAndRealtimeBehavior>` section of the User Manual.
...
enable the
`lookahead <PageOtherTimeframesAndData_CommonCharacteristics_Lookahead>` parameter by setting it to
...
`lower timeframes <PageOtherTimeframesAndData_RequestSecurity_Timeframes_LowerTimeframes>` as well, with limitations. When accessing data from a lower
...
`lookahead <PageOtherTimeframesAndData_CommonCharacteristics_Lookahead>` parameter:
[barmerge.lookahead_on](https://www.tradingview.com/pine-script-reference/v5/#const_barmerge.lookahead_on)
...
prevent `repainting <PageRepainting>`---a
...
`array <PageArrays>` containing data from
...
`this FAQ entry <PageFaq_OtherDataAndTimeframes_HowDoIDefineAHigherIntervalThatIsAMultipleOfTheCurrentOne>`.
...
`table <PageTables>` on the chart\'s top
...
`table <PageTables>` at the top right of
...
`Data feeds <PageOtherTimeframesAndData_DataFeeds>` section in the User Manual.
...
resulting in `repainting <PageRepainting>`
values.
:::

For methods to retrieve non-repainting higher timeframe values, see this
relevant
`FAQ entry <PageFaq_OtherDataAndTimeframes_HowToAvoidRepaintingWhenUsingTheRequestSecurityFunction>`, the
`Avoiding Repainting <PageOtherTimeframesAndData_HistoricalAndRealtimeBehavior_AvoidingRepainting>` section, and the section on
...
uses a
`ternary operator <PageOperators_TernaryOperator>` to return
[na](https://www.tradingview.com/pine-script-reference/v5/#var_na),
...
Note that:
    ...
    -   We use
        ...
        -   When the chart\'s timeframe is shorter than the timeframe
            ...
            appears in a `table <PageTables>` located at the bottom right of the chart.
    ...
    -   To manage calls to higher timeframes, the script uses a custom
        ...
        -   When `repaint` is set to false, the function sets the
            ...
            outlined in the
            `Avoiding Repainting <PageOtherTimeframesAndData_HistoricalAndRealtimeBehavior_AvoidingRepainting>` section of the User Manual.
...
-   `Other Timeframes and Data <PageOtherTimeframesAndData>`
-   `` \`request.security()\` <PageOtherTimeframesAndData_RequestSecurity> ``
-   `Avoiding Repainting <PageOtherTimeframesAndData_HistoricalAndRealtimeBehavior_AvoidingRepainting>`
-   `` Repainting \`request.security()\` calls <PageRepainting_HistoricalVsRealtimeCalculations_RepaintingRequestSecurityCalls> ``

## Programming 

-   `What does scope mean? <PageFaq_Programming_WhatDoesScopeMean>`
-   `How can I convert a script from one version to another? <PageFaq_Programming_HowCanIConvertAScriptFromOneVersionToAnother>`
-   `Can I access the source code of 'Invite-Only' or 'closed source' scripts? <PageFaq_Programming_CanIAccessTheSourceCodeOfInviteOnlyOrClosedSorceScripts>`
-   `Is Pine Script™ an object-oriented language? <PageFaq_Programming_IsPineScriptAnObjectOrientedLanguage>`
-   `How can I access the built-in indicators' source code? <PageFaq_Programming_HowCanIAccessTheBuiltInIndicatorsSourceCode>`
-   `How can I examine the value of a string in my script? <PageFaq_Programming_HowCanIExamineTheValueOfAStringInMyScript>`
-   `How can I visualize my script's conditions on the chart? <PageFaq_Programming_HowCanIVisualizeMyScriptsConditionsOnTheChart>`
-   `How can I make the console appear in the editor? <PageFaq_Programming_HowCanIMakeTheConsoleAppearInTheEditor>`
-   `How can I plot numeric values so that they do not disrupt the indicator's scale? <PageFaq_Programming_HowCanIPlotNumericValuesSoThatTheyDoNotDisruptTheIndicatorsScale>`
...
-   **Upgrading from version 2 to version 3 -** The transition from v2
    ...
    `link to the conversion article <PageToPineVersion3>` that would provide you with helpful guidance.
...
`publishing scripts <PagePublishing>` in
...
`user-defined types <PageTypeSystem_UserDefinedTypes>` (UDTs). In Pine Script™, UDTs enable the creation of
`objects <PageObjects>`. These objects are
...
`debugging strings <PageDebugging_Strings>`.
...
See
`this <PageDebugging_Conditions_CompoundAndNestedConditions>` section of the Pine Script™ User Manual on debugging
compound conditions.
...
See
`this <PageDebugging_NumericValues_PlottingNumbers_WithoutAffectingTheScale>` section of the Pine Script™ User Manual on displaying
...
-   `Strategy basics <PageFaq_Strategies_StrategyBasics>`
    -   `How can I turn my indicator into a strategy? <PageFaq_Strategies_StrategyBasics_HowCanITurnMyIndicatorIntoAStrategy>`
    -   `How do I set a stop loss? <PageFaq_Strategies_StrategyBasics_HowDoISetAStopLoss>`
    -   `How can I save the entry price in a strategy? <PageFaq_Strategies_StrategyBasics_HowCanISaveTheEntryPriceInAStrategy>`
    -   `How do I implement date/time range filtering in strategies? <PageFaq_Strategies_StrategyBasics_HowDoIImplementDateTimeRangeFilteringInStrategies>`
-   `Order execution and management <PageFaq_Strategies_OrderExecutionAndManagement>`
    -   `Why are my orders executed on the bar following my triggers? <PageFaq_Strategies_OrderExecutionAndManagement_WhyAreMyOrdersExecutedOnTheBarFollowingMyTriggers>`
    -   `How can I set up multiple take profit levels to gradually close out a position? <PageFaq_Strategies_OrderExecutionAndManagement_HowCanISetUpMultipleTakeProfitLevelsToGraduallyCloseOutAPosition>`
    -   `How can I execute a trade midway through a bar before it fully closes? <PageFaq_Strategies_OrderExecutionAndManagement_HowCanIExecuteATradeMidwayThroughABarBeforeItFullyCloses>`
    -   `How can I set up a trade to exit during the same bar in which the entry occurred? <PageFaq_Strategies_OrderExecutionAndManagement_HowCanISetUpATradeToExitDuringTheSameBarInWhichTheEntryOccured>`
-   `Advanced order types and conditions <PageFaq_Strategies_AdvancedOrderTypesAndConditions>`
    -   `How can I set a stop loss and take profit level as a percentage from my entry point? <PageFaq_Strategies_AdvancedOrderTypesAndConditions_HowCanISetAStopLossAndTakeProfitLevelAsAPercentageFromMyEntryPoint>`
    -   `How do I move my stop loss to breakeven? <PageFaq_Strategies_AdvancedOrderTypesAndConditions_HowDoIMoveMyStopLossToBreakeven>`
    -   `How do I implement a trailing stop loss? <PageFaq_Strategies_AdvancedOrderTypesAndConditions_HowDoIImplementATrailingStopLoss>`
    -   `How can I set a time-based condition to close out a position? <PageFaq_Strategies_AdvancedOrderTypesAndConditions_HowCanISetATimeBasedConditionToCloseOutAPosition>`
    -   `How can I configure a bracket order with a specific risk-to-reward (R:R) ratio? <PageFaq_Strategies_AdvancedOrderTypesAndConditions_HowCanIConfigureABracektOrderWithASpecificRiskToRewardRRRatio>`
    -   `How can I adjust my position size to ensure that I risk a fixed percentage of my equity? <PageFaq_Strategies_AdvancedOrderTypesAndConditions_HowCanIAdjustMyPositionSizeToEnsureThatIRiskAFixedPercentageOfMyEquity>`
-   `Strategy optimization and testing <PageFaq_Strategies_StrategyOptimizationAndTesting>`
    -   `Why did my trade results change dramatically overnight? <PageFaq_Strategies_StrategyOptimizationAndTesting_WhyDidMyTradeResultsChangeDramaticallyOvernight>`
    -   `Why is backtesting on Heikin Ashi and other non-standard charts not recommended? <PageFaq_Strategies_StrategyOptimizationAndTesting_WhyIsBacktestingOnHeikenAshiAndOtherNonStandardChartsNotRecommended>`
    -   `How can I backtest deeper into history? <PageFaq_Strategies_StrategyOptimizationAndTesting_HowCanIBacktestDeeperIntoHistory>`
    -   `How do I backtest multiple symbols at once? <PageFaq_Strategies_StrategyOptimizationAndTesting_HowDoIBacktestMultipleSymbolsAtOnce>`
    -   `What does Bar Magnifier do? <PageFaq_Strategies_StrategyOptimizationAndTesting_WhatDoesBarMagnifierDo>`
-   `Advanced features and integration <PageFaq_Strategies_AdvancedFeaturesAndIntegration>`
    -   `Can my strategy script place orders with TradingView brokers? <PageFaq_Strategies_AdvancedFeaturesAndIntegration_CanMyStrategyScriptPlaceOrdersWithTradingviewBrokers>`
    -   `How can I implement a time delay between orders? <PageFaq_Strategies_AdvancedFeaturesAndIntegration_HowCanIImplementATimeDelayBetweenOrders>`
    -   `How can I calculate custom statistics in a strategy? <PageFaq_Strategies_AdvancedFeaturesAndIntegration_HowCanICalculateCustomStatisticsInAStrategy>`
    -   `How do I incorporate leverage into my strategy? <PageFaq_Strategies_AdvancedFeaturesAndIntegration_HowDoIIncorporateLeverageIntoMyStrategy>`
    -   `Can you hedge in a Pine Script strategy? <PageFaq_Strategies_AdvancedFeaturesAndIntegration_CanYouHedgeInAPineScriptStrategy>`
    -   `Can I connect my strategies to my paper trading account? <PageFaq_Strategies_StrategyOptimizationAndTesting_CanIConnectMyStrategiesToMyPaperTradingAccount>`
-   `Troubleshooting and specific issues <PageFaq_Strategies_TroubleshootingAndSpecificIssues>`
    -   `Why are no trades executed after I added the strategy to the chart? <PageFaq_Strategies_TroubleshootingAndSpecificIssues_WhyAreNoTradesExecutedAfterIAddedTheStrategyToTheChart>`
    -   `Why is my strategy repainting? <PageFaq_Strategies_TroubleshootingAndSpecificIssues_WhyIsMyStrategyRepainting>`
    -   `How do I turn off alerts specifically for my stop loss and take profit orders? <PageFaq_Strategies_TroubleshootingAndSpecificIssues_HowDoITurnOffAlertsSpecificallyForMyStopLossAndTakeProfitOrders>`
...
-   Pine Script™ strategies support
    `various types <PageStrategies_OrdersAndEntries_OrderTypes>` of orders including market, limit, stop, and stop-limit
    orders, allowing users to simulate different trading scenarios.
-   When a strategy using
    `order placement commands <PageStrategies_OrdersAndEntries_OrderPlacementCommands>` is applied to the chart, it uses the
    `broker emulator <PageStrategies_BrokerEmulator>` to calculate hypothetical trades and displays the
    results in the
    `Strategy Tester <PageStrategies_StrategyTester>` tab.
...
2.  **Incorporate strategy order placement commands:** Add strategy
    `order functions <PageStrategies_OrdersAndEntries_OrderPlacementCommands>` for executing orders, dictating the type of orders to
...
`order placement commands <PageStrategies_OrdersAndEntries_OrderPlacementCommands>` that align with your trading objectives, and apply them
...
`User Manual <PageStrategies_OrdersAndEntries_OrderPlacementCommands_StrategyExit>`.
...
2.  **Setting up a trailing stop loss:** There are two methods to create
    ...
    `trailing stop loss <PageFaq_Strategies_AdvancedOrderTypesAndConditions_HowDoIImplementATrailingStopLoss>` for further guidance.
3.  **Creating scaled exits at different prices:** For executing exits
    ...
    `multiple exits <PageFaq_Strategies_OrderExecutionAndManagement_HowCanISetUpMultipleTakeProfitLevelsToGraduallyCloseOutAPosition>` for more information.
4.  **Moving a stop loss to breakeven:** Adjust your stop loss to the
    ...
    `moving stop losses to breakeven <PageFaq_Strategies_AdvancedOrderTypesAndConditions_HowDoIMoveMyStopLossToBreakeven>`.
5.  **Adjusting position size based on stop loss:** Modify your position
    ...
    `position sizing <PageFaq_Strategies_AdvancedOrderTypesAndConditions_HowCanIAdjustMyPositionSizeToEnsureThatIRiskAFixedPercentageOfMyEquity>`.
...
-   When implementing strategies that involve multiple entries or
    ...
    `closing a market position <PageStrategies_ClosingAMarketPosition>`, particularly when trades are exited individually. The
...
to
`this <PageFaq_Strategies_AdvancedOrderTypesAndConditions_HowCanISetAStopLossAndTakeProfitLevelAsAPercentageFromMyEntryPoint>` FAQ question.
...
3.  **Broker Emulator synchronization:** When configuring alerts for
    `executing real trades <PageFaq_Alerts_CanMyPineStrategyOrIndicatorPlaceAutomatedOrdersInMarkets>`, initiate the date filter\'s start date to today\'s
...
-   Within the `timeWithinAllowedRange()` function, we employ the
    ...
    `Sessions <PageSessions>` section in
    the User Manual.
...
When using Pine Script™ `strategies <PageStrategies>`, it\'s common for traders to notice their orders being
...
-   To maintain consistent behavior between realtime and historical
    ...
    bar, which minimizes `repainting <PageRepainting>`.
...
enabling the
`process_orders_on_close <PageStrategies_AlteringCalculationBehavior_ProcessOrdersOnClose>` setting. By setting this parameter to `true`, the broker
...
`alter a strategy's calculation behavior <PageStrategies_AlteringCalculationBehavior>` by enabling the `process_orders_on_close` setting, but be
...
2.  Utilizing an
    `OCA reduce group <PageStrategies_OCAGroups_StrategyOCAReduce>`. This method is ideal when you have a non-uniform number
...
`third-party order executions <PageFaq_Alerts_CanMyPineStrategyOrIndicatorPlaceAutomatedOrdersInMarkets>`, as each order at the stop level would generate a separate
...
Here, an `OCA group <PageStrategies_OCAGroups>` ensures that only one stop order is used to close out the
...
-   **Realtime data:**
    ...
    (`calc_on_every_tick <PageStrategies_AlteringCalculationBehavior_CalcOnEveryTick>`). This setting allows the strategy to process each
...
`repainting <PageRepainting>`. This is
...
`process_orders_on_close <PageStrategies_AlteringCalculationBehavior_ProcessOrdersOnClose>`, but it is specific to the `strategy.close()` function.
...
`repainting <PageRepainting>`. To learn
more about why orders are typically simulated at the opening of the next
bar in Pine Script™, please refer to
`this FAQ entry <PageFaq_Strategies_OrderExecutionAndManagement_WhyAreMyOrdersExecutedOnTheBarFollowingMyTriggers>`.
:::
...
by setting the
`calc_on_order_fills <PageStrategies_AlteringCalculationBehavior_CalcOnOrderFills>` argument of the
[strategy()](https://www.tradingview.com/pine-script-reference/v5/#fun_strategy)
...
Note that:
    ...
    -   The built-in trailing order logic relies on the
        `broker emulator <PageStrategies_BrokerEmulator>` to make simplified assumptions about price movements
        ...
        `repainting <PageRepainting>`.
    -   [Premium](https://www.tradingview.com/pricing/) account holders
        can use the
        `bar magnifier <PageBrokerEmulator_BarMagnifier>` feature to analyze data from smaller timeframes than
...
-   **Realtime vs. historical bars:** In realtime, the same logic
    ...
    `calc_on_every_tick <PageStrategies_AlteringCalculationBehavior_CalcOnEveryTick>`. This modification allows the trade to close as soon as
    the first tick exceeds the timeout value after trade entry. Remember
    that
    `altering emulator behavior <PageStrategies_AlteringCalculationBehavior>` typically introduces
    `repainting <PageRepainting>`.
...
-   Leverage the `timeNow` variable in the `closePositionsAfter()`
    ...
    Script\'s `execution model <PageExecutionModel>` where scripts calculate once per historical bar at the
    close.
-   Utilize `built-in <PageBuiltInFunctions>` functions in the `strategy.opentrades.*()` namespace. We
    use
...
`process_orders_on_close <PageStrategies_AlteringCalculationBehavior_ProcessOrdersOnClose>`, ensuring that the order is simulated at the bar\'s close.
...
behavior, see
`this <PageFaq_Strategies_OrderExecutionAndManagement_WhyAreMyOrdersExecutedOnTheBarFollowingMyTriggers>` relevant FAQ entry.
:::
...
`starting point <PageRepainting_DatasetVariations_StartingPoints>` of the chart\'s historical data. Understanding why this
...
to a natural `repainting <PageRepainting>`
...
1.  **Export strategy results:** Regularly [exporting your strategy
    ...
    *export data* option in the top of the *List of Trades* tab in the
    `strategy tester <PageStrategies_StrategyTester>`.
...
are not suited for
`strategy backtesting <PageStrategies_NotesOnTestingStrategies>` or automated trading systems execution, for several reasons:
...
-   **Chart visibility limitation:** While deep backtesting provides
    ...
    `Strategy Tester's <PageStrategies_StrategyTester>` report for analysis, which may not offer the same
...
-   **Exporting strategy tester results:** While direct multi-symbol
    ...
    results from the
    `Strategy Tester <PageStrategies_StrategyTester>`. This feature is beneficial for maintaining records and
...
The `Bar Magnifier <PageBrokerEmulator_BarMagnifier>` feature, available for TradingView
[Premium](https://www.tradingview.com/pricing/) account holders,
...
the `broker emulator <PageStrategies_BrokerEmulator>` must make on price movement using only a single set of OHLC
...
`User Manual <PageBrokerEmulator_BarMagnifier>`. These examples illustrate the difference this setting
...
to
`this <PageFaq_Alerts_CanMyPineStrategyOrIndicatorPlaceAutomatedOrdersInMarkets>` related FAQ entry.
...
-   Draw an arrow-styled
    `line <PageLinesAndBoxes_Lines>` and a
    `label <PageTextAndShapes_Labels>`
...
1.  **Initiating strategy orders:** To calculate custom statistics in a
    ...
    `order placement commands <PageStrategies_OrdersAndEntries_OrderPlacementCommands>`, which mimics sending orders to a broker or exchange.
...
-   Utilize a position-sizing method to manage risk by adjusting the
    ...
    the method shown
    `in this FAQ entry <PageFaq_Strategies_AdvancedOrderTypesAndConditions_HowCanIAdjustMyPositionSizeToEnsureThatIRiskAFixedPercentageOfMyEquity>`.
-   Incorporate trading costs. To ensure that a strategy performance
    ...
    Read more on that
    `here <PageStrategies_SimulatingTradingCosts>`.
-   Round the
    [open](https://www.tradingview.com/pine-script-reference/v5/#var_open),
    [high](https://www.tradingview.com/pine-script-reference/v5/#var_high),
    [low](https://www.tradingview.com/pine-script-reference/v5/#var_low)
    ...
    `Strategy Tester <PageStrategies_StrategyTester>` or with strategy order-related built-ins. We explain the
    implications of this further
    `here <PageFaq_VariablesAndOperators_WhyDoTheOhlcBuiltInsSometimesReturnDifferentValuesThanTheOnesShownOnTheChart>`.
...
-   A practical workaround to simulate trading, similar to paper
    trading, involves using `alerts <PageAlerts>` within strategies. These alerts are set up to generate
...
`date filter <PageFaq_Strategies_StrategyBasics_HowDoIImplementDateTimeRangeFilteringInStrategies>` initiating the date filter\'s start date to today\'s date.
...
If you\'ve implemented a `strategy <PageStrategies>` and added it to your chart but observe that it doesn\'t
generate orders, leading to \"No data\" displayed in the \"List of
Trades\" and \"Overview\" tabs of the
`Strategy Tester <PageStrategies_StrategyTester>`, consider the following potential reasons and their
solutions:
...
-   **Lack of order placement commands:** Your strategy must use
    ...
    `order placement commands <PageStrategies_OrdersAndEntries_OrderPlacementCommands>` section in the User Manual for more information.
...
-   **\`calc_on_every_tick\`:** When the
    `calc_on_every_tick <PageStrategies_AlteringCalculationBehavior_CalcOnEveryTick>` setting is enabled in a strategy script, it recalculates
...
-   **\`calc_on_order_fills\`:** The
    `calc_on_order_fills <PageStrategies_AlteringCalculationBehavior_CalcOnOrderFills>` setting triggers a strategy to recalculate immediately
    ...
    `this one <PageFaq_Strategies_AdvancedOrderTypesAndConditions_HowCanISetAStopLossAndTakeProfitLevelAsAPercentageFromMyEntryPoint>`), rather than waiting for the bar to close. However,
...
-   **Incorrect alert type:** Each `strategy.*()` order function comes
    with integrated `alert <PageAlerts>`
    ...
    `this FAQ <PageFaq_Alerts_HowDoIMakeAnAlertAvailableFromMyScript>` entry.
-   **Using \`request.security()\`:** When requesting values from a
    ...
    `Avoiding repainting <PageOtherTimeframesAndData_HistoricalAndRealtimeBehavior_AvoidingRepainting>` for detailed guidance.
...
-   **Disabling alerts for specific orders:** To prevent alerts from
    ...
    `order placement commands <PageStrategies_OrdersAndEntries_OrderPlacementCommands>` provide a parameter called `disable_alert`. By setting
...
-   `How can I place text on the chart? <PageFaq_StringsAndFormatting_HowCanIPlaceTextOnTheChart>`
-   `How can I position text on either side of a single bar? <PageFaq_StringsAndFormatting_HowCanIPositionTextOnEitherSideOfASingleBar>`
-   `How can I lift plotshape() text up? <PageFaq_StringsAndFormatting_HowCanILiftPlotshapeTextUp>`
-   `How can I print a value at the top right of the chart? <PageFaq_StringsAndFormatting_HowCanIPrintAValueAtTheTopRightOfTheChart>`
-   `How can I split a string into characters? <PageFaq_StringsAndFormatting_HowCanISplitAStringIntoCharacters>`
...
-   `Labels <PageTextAndShapes_Labels>`,
    when you need to display dynamic text that may vary bar to bar.
-   `Tables <PageTables>`, for showcasing
...
`Labels <PageTextAndShapes_Labels>` are
...
`Tables <PageTables>` are useful to
...
`table <PageTables>` for this purpose.
...
-   `How can I trigger a condition only when a number of bars have elapsed since the last condition occurred? <PageFaq_Techniques_HowCanITriggerAConditionOnlyWhenANumberOfBarsHaveElapsedSinceTheLastConditionOccured>`
-   `How can my script identify what chart type is active? <PageFaq_Techniques_HowCanMyScriptIdentifyWhatChartTypeIsActive>`
-   `How can I plot the chart's visible high and low? <PageFaq_Techniques_HowCanIPlotTheChartsVisibleHighAndLow>`
-   `How can I remember the last time a condition occurred? <PageFaq_Techniques_HowCanIRememberTheLastTimeAConditionOccured>`
-   `How can I plot the previous and current day's open? <PageFaq_Techniques_HowCanIPlotThePreviousAndCurrentDaysOpen>`
-   `How can I count the occurrences of a condition in the last x bars? <PageFaq_Techniques_HowCanICountTheOccurencesOfAConditionInTheLastXBars>`
-   `How can I implement an On/Off switch? <PageFaq_Techniques_HowCanIImplementAnOnOffSwitch>`
-   `How can I allow transitions from condition A►B or B►A, but not A►A nor B►B? <PageFaq_Techniques_HowCanIAllowTransitionsFromConditionABorBAButNotAANorBB>`
-   `Can I merge two or more indicators into one? <PageFaq_Techniques_CanIMergeTwoOrMoreIndicatorsIntoOne>`
-   `How can I rescale an indicator from one scale to another? <PageFaq_Techniques_HowCanIRescaleAnIndicatorFromOneScaleToAnother>`
-   `How can I calculate my script's run time? <PageFaq_Techniques_HowCanICalculateMyScriptsRunTime>`
-   `How can I save a value when an event occurs? <PageFaq_Techniques_HowCanISaveAValueWhenAnEventOccurs>`
-   `How can I count touches of a specific level? <PageFaq_Techniques_HowCanICountTouchesOfASpecificLevel>`
-   `How can I know if something is happening for the first time since the beginning of the day? <PageFaq_Techniques_HowCanIKnowIfSomethingIsHappeningForTheFirstTimeSinceTheBeginingOfTheDay>`
-   `How can I optimize Pine Script™ code? <PageFaq_Techniques_HowCanIOptimizePineScriptCode>`
-   `How can I access a stock's financial information using Pine Script™? <PageFaq_Techniques_HowCanIAccessAStocksFinancialInformationUsingPineScript>`
-   `How can I find the maximum value among the last pivots? <PageFaq_Techniques_HowCanIFindTheMaximumValueAmongTheLastPivots>`
-   `How can I display plot values in the chart's scale? <PageFaq_Techniques_HowCanIDisplayPlotValuesInTheChartsScale>`
-   `How can I reset a sum on a condition? <PageFaq_Techniques_HowCanIResetASumOnACondition>`
-   `How can I accumulate a value for two exclusive states? <PageFaq_Techniques_HowCanIAccumulateAValueForTwoExclusiveStates>`
-   `How can I organize my script's inputs in the Settings/Inputs tab? <PageFaq_Techniques_HowCanIOrganizeMyScriptsInputsInTheSettingsInputsTab>`
...
`line <PageLinesAndBoxes_Lines>`,
`label <PageTextAndShapes_Labels>`, or
`box <PageLinesAndBoxes_Boxes>` should not
...
`Repainting <PageRepainting>` in the
...
-   **Alerts on bar close:** Configure alerts to trigger at the close of
    ...
    `this FAQ entry <PageFaq_Alerts_WhyIsMyAlertFiringWhenItsNotSupposedTo>`.
...
`User Manual <PageRepainting>` addresses
...
the `chart.*` `built-ins <PageBuiltInFunctions>` to identify the chart type and convert this information into
...
`lines <PageLinesAndBoxes_Lines>` on the
chart, signifying the highest and lowest price points within the range
of visible bars. `Labels <PageTextAndShapes_Labels>` are generated for these lines, displaying both the price and
...
`global scope <PageFaq_Programming_WhatDoesScopeMean>` and outside the local block defined by
[barstate.islast](https://www.tradingview.com/pine-script-reference/v5/#var_barstate.islast),
...
`"series" <PageTypeSystem_Qualifiers_Series>` value, i.e., a value that can change on each bar. This
...
`simple <PageTypeSystem_Qualifiers_Simple>` or
`series <PageTypeSystem_Qualifiers_Series>` length.
...
The `var <PageVariableDeclarations_Var>`
keyword in Pine Script™ ensures a variable is only initialized once. If
it\'s declared in the
`global scope <PageFaq_Programming_WhatDoesScopeMean>`, the initialization happens on the first bar. If it\'s
...
with the `var <PageVariableDeclarations_Var>` keyword is only initialized once at
[bar_index](https://www.tradingview.com/pine-script-reference/v5/#var_bar_index)
...
`var <PageVariableDeclarations_Var>` and
...
`:= <PageOperators_ReassignmentOperator>`
...
`if <PageConditionalStructures_If>` block.
...
1.  **Understand the Pine Script™ execution model**
    ...
    -   Refer to the User Manual sections on the
        `execution model <PageExecutionModel>` and `time series <PageTimeSeries>` for guidance.
...
3.  **Utilize built-in functions**
    -   Prefer `built-in <PageBuiltInFunctions>` functions for calculating values to maximize
        efficiency.
...
8.  **Isolate large code sections**
    ...
    -   Move functions to `libraries <PageLibraries>` and import them when necessary. This approach can
...
9.  **Use the var keyword for complex initializations**
    -   Declare variables with the
        `var <PageVariableDeclarations_Var>` keyword when initialization involves time-consuming
...
12. **Utilize alternative techniques to avoid specific functions**
    ...
    -   Consider techniques similar to the one mentioned
        `here <PageFaq_Techniques_HowCanISaveAValueWhenAnEventOccurs>` to bypass the use of
        [ta.valuewhen()](https://www.tradingview.com/pine-script-reference/v5/#fun_ta.valuewhen).
    -   Implement approaches resembling
        `method 4 <PageFaq_Techniques_HowCanIRememberTheLastTimeAConditionOccured>` to circumvent the use of
        [ta.barssince()](https://www.tradingview.com/pine-script-reference/v5/#fun_ta.barssince).
...
3.  Through the
    ...
    a.  Your script must allow for an
        `external input <PageFaq_Indicators_HowCanIUseOneScriptsOutputAsAnInputIntoAnother>`.
...
`array <PageArrays>` simplifies both the
process and the code significantly.

The array is managed in a way that mimics a
`queue <PageArrays_InsertingAndRemovingArrayElements_UsingAnArrayAsAQueue>`: each time a new pivot is identified, the oldest element is
...
Note that:
    ...
    -   In the
        ...
        in the `tuple <PageTypeSystem_Tuples>`. To avoid unnecessary variable declaration and to
...
-   `How can I get the time of the first bar in the dataset? <PageFaq_TimesDatesSessions_HowCanIGetTheTimeOfTheFirstBarInTheDataset>`
-   `How can I convert a time to a date-time string? <PageFaq_TimesDatesSessions_HowCanIConvertATimeToADatetimeString>`
-   `How can I know how many days are in the current month? <PageFaq_TimesDatesSessions_HowCanIKnowHowManyDaysAreInTheCurrentMonth>`
-   `How can I detect the chart's last day? <PageFaq_TimesDatesSessions_HowCanIDetectTheChartsLastDay>`
-   `How can I plot a value starting n months/years back? <PageFaq_TimesDatesSessions_HowCanIPlotAValueStartingNMonthsYearsBack>`
-   `How can I track highs/lows for a specific timeframe? <PageFaq_TimesDatesSessions_HowCanITrackHighsLowsForASpecificTimeframe>`
-   `How can I track the highs/lows within a specific session or time of day? <PageFaq_TimesDatesSessions_HowCanITrackTheHighsLowsWithinASpecificSessionOrTimeOfDay>`
-   `How can I track highs/lows between specific intrabar hours? <PageFaq_TimesDatesSessions_HowCanITrackHighsLowsBetweenSpecificIntrabarHours>`
-   `How can I detect a specific date/time? <PageFaq_TimesDatesSessions_HowCanIDetectASpecificDateTime>`
-   `How can I know the date when the highest value was found? <PageFaq_TimesDatesSessions_HowCanIKnowTheDateWhenTheHighestValueWasFound>`
-   `How can I detect bars opening at a specific hour? <PageFaq_TimesDatesSessions_HowCanIDetectBarsOpeningAtASpecificHour>`
-   `Can I time the duration of a condition? <PageFaq_TimesDatesSessions_CanITimeTheDurationOfACondition>`
-   `How can I identify the nth occurrence of a weekday in the month? <PageFaq_TimesDatesSessions_HowCanIIdentifyTheNthOccurrenceOfAWeekdayInTheMonth>`
-   `How can I implement a countdown timer? <PageFaq_TimesDatesSessions_HowCanIImplementACountdownTimer>`
-   `How can I get the week of the month? <PageFaq_TimesDatesSessions_HowCanIGetTheWeekOfTheMonth>`
...
`built-in <PageBuiltInFunctions>` chart
...
-   Create a function, `hiLoBetweenTime()`, to track the session\'s
    ...
    -   Declare two variables, `hi` and `lo`, using the
        `var <PageVariableDeclarations_Var>` keyword. This ensures that their values are retained
        bar to bar.
    -   Set the `hi` and `lo` variables on the first bar of the session
...
-   `What is the variable name for the current price? <PageFaq_VariablesAndOperators_WhatIsTheVariableNameForTheCurrentPrice>`
...
-   `What's the difference between ==, =, and :=? <PageFaq_VariablesAndOperators_WhatsTheDifferenceBetween===And:=>`
-   `Can I use the := operator to assign values to past values of a series? <PageFaq_VariablesAndOperators_CanIUseThe:=OperatorToAssignValuesToPastValuesOfASeries>`
-   `Why do the OHLC built-ins sometimes return different values than the ones shown on the chart? <PageFaq_VariablesAndOperators_WhyDoTheOhlcBuiltInsSometimesReturnDifferentValuesThanTheOnesShownOnTheChart>`
...
`history-referencing operator <PageOperators_HistoryReferencingOperator>` in Pine Script™.
...
`varip <PageVariableDeclarations_Varip>`
...
`typical behavior <PageExecutionModel>` of
...
The `"=" operator <PageOperators_AssignmentOperator>` is used for declaring and initializing variables, assigning
a specific value to a named variable. For example, `a = 0` sets the
variable `a` to hold the value 0.

The
`":=" operator <PageOperators_ReassignmentOperator>` is used for reassigning values to already initialized
...
is a
`comparison operator <PageOperators_ComparisonOperators>`. It checks the equality between two values, returning a
`boolean <PageTypeSystem_Types_Bool>`
...
the `built-in <PageBuiltInFunctions>` OHLC
variables
([open](https://www.tradingview.com/pine-script-reference/v5/#var_open),
[high](https://www.tradingview.com/pine-script-reference/v5/#var_high),
[low](https://www.tradingview.com/pine-script-reference/v5/#var_low),
...
`conditional structure <PageConditionalStructures>` or
`ternary operator <PageOperators_TernaryOperator>` will execute, which may cause unpredictable results. Thus,
...
`handling of na values <PageTypeSystem_NaValue>`. Specifically, they demonstrate how to check for `na` values
using the
[na()](https://www.tradingview.com/pine-script-reference/v5/#fun_na)
...
-   `Why can't I use a plot in an if or for statement? <PageFaq_Visuals_WhyCantIUseAPlotInAnIfOrForStatement>`
-   `Can I plot diagonals between two points on the chart? <PageFaq_Visuals_CanIPlotDiagonalsBetweenTwoPointsOnChart>`
-   `How do I plot a line using start/stop criteria? <PageFaq_Visuals_HowDoIPlotALineUsingStartStopCriteria>`
-   `How do I plot a support or a trend line? <PageFaq_Visuals_HowDoIPlotASupportOrATrendLine>`
-   `How can I use colors in my indicator plots? <PageFaq_Visuals_HowCanIUseColorsInMyIndicatorPlots>`
-   `How do I make my indicator plot over the chart? <PageFaq_Visuals_HowDoIMakeMyIndicatorPlotOverTheChart>`
-   `How can I plot vertical lines on a chart? <PageFaq_Visuals_HowCanIPlotVerticalLinesOnAChart>`
-   `How can I toggle hline() plots on and off? <PageFaq_Visuals_HowCanIToggleHlinePlotsOnAndOff>`
-   `How can I draw lines or labels into the future? <PageFaq_Visuals_HowCanIDrawLinesOrLabelsIntoTheFuture>`
-   `How can I keep only the last n number of objects? <PageFaq_Visuals_HowCanIKeepOnlyTheLastNNumberOfObjects>`
-   `Is it possible to draw geometric shapes? <PageFaq_Visuals_IsItPossibleToDrawGeometricShapes>`
...
[for](https://www.tradingview.com/pine-script-reference/v5/#kw_for)
statements or other
`local scopes <PageFaq_Programming_WhatDoesScopeMean>` isn\'t permitted. This is because the system needs to know
...
3.  Use the conditional value or color in the `plot()` function that is
    called in the
    `global scope <PageFaq_Programming_WhatDoesScopeMean>` of your script.
...
-   Use an array-based
    `queue <PageArrays_InsertingAndRemovingArrayElements_UsingAnArrayAsAQueue>` to continuously store the two most recent pivot points.
...
4.  **Implement a queue:**
    -   Manage the array like a
        `queue <PageArrays_InsertingAndRemovingArrayElements_UsingAnArrayAsAQueue>` to maintain only a maximum number of lines at a
        time.
...
1.  **Identify pivot points:**
    -   We use the `built-in <PageBuiltInFunctions>` functions
        [ta.pivotlow()](https://www.tradingview.com/pine-script-reference/v5/#fun_ta.pivotlow)
        and
        [ta.pivothigh()](https://www.tradingview.com/pine-script-reference/v5/#fun_ta.pivothigh)
...
4.  **Implement a queue:**
    -   Just as we did for support and resistance lines, we make use of
        a
        `queue <PageArrays_InsertingAndRemovingArrayElements_UsingAnArrayAsAQueue>` with our `arrayQueue()` function to ensure our array
...
1.  **Predefined colors:** Without needing to specify hexadecimal or RGB
    ...
    `color constants <PageColors_ConstantColors>` like
...
1.  **Using bar_index:**
    ...
    -   **Disadvantage: Limitation on number of bars**
        -   While `bar_index` is straightforward, it has a constraint of
            a
            `maximum of 500 bars <PageLimitations_OtherLimitations_MaximumBarsForward>` into the future. Exceeding this will result in a
            runtime error.
...
2.  **Initialize visual elements:**
    -   We use the
        `var <PageVariableDeclarations_Var>` keyword to declare two lines and two labels,
...
utilizing an `array <PageArrays>` to store
these references.
...
`page on arrays <PageArrays_InsertingAndRemovingArrayElements_UsingAnArrayAsAQueue>`, but we will implement a function that allows us to save
lines:
...
1.  Create the user input `labelQtyInput` for the desired quantity of
    ...
    `maximum of 500 <PageLimitations_ChartVisuals_LineBoxPolylineAndLabelLimits>`. The default is set to 50.
...
3.  Declare an array named `labelsArray` to store label references with
    the `var <PageVariableDeclarations_Var>` keyword to ensure it is initialized only once and
...
`local scopes <PageFaq_Programming_WhatDoesScopeMean>` to queue elements conditionally.
...
and `lines <PageLinesAndBoxes_Lines>`.
...
1.  **Drawing with Polylines**
    ...
    `limitation <PageLinesAndBoxes_Limitations>` within any given script to include no more than 100
    polyline objects.
...
2.  **Drawing with Lines**
    ...
    polylines. A `line <PageLinesAndBoxes_Lines>` in Pine Script™ is a simple, straight connection between
