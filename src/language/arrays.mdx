...
or a
`user-defined type <PageTypeSystem_UserDefinedTypes>`, always qualified as \"series\". Scripts reference arrays
...
Where `<type>` is a
`type template <PageTypeSystem_TypeTemplates>` for the array that declares the type of values it will
...
`type template <PageTypeSystem_TypeTemplates>`. Alternatively, we can use the `type` name followed by the
...
collections (arrays, `matrices <PageMatrices>`) of
these types.
:::
...
`conditional structures <PageConditionalStructures>`. Unlike some of the other built-in types, namely
...
`time series <PageTypeSystem_TimeSeries>`,
...
See the `Looping <PageArrays_Looping>`
