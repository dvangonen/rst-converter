...
sidebar-title: Conditional structures
page-title: Language / Conditional structures
---
...
[for](https://www.tradingview.com/pine-script-reference/v5/#kw_for) and
...
Some Pine Script™ built-in functions cannot be called from within the
...
An [if](https://www.tradingview.com/pine-script-reference/v5/#kw_if)
...
-   \<expression\> must be of \"bool\" type or be auto-castable to that
    ...
    the `Type system <PageTypeSystem_Types>` page).
...
When no \<expression\> has evaluated to
[true](https://www.tradingview.com/pine-script-reference/v5/#const_true)
and no `else` clause exists,
[na](https://www.tradingview.com/pine-script-reference/v5/#var_na) is
returned.

Using [if](https://www.tradingview.com/pine-script-reference/v5/#kw_if)
...
-   The expression we are switching on is the variable `maType`, which
    ...
    \"`input <PageTypeSystem_Qualifiers_Input>`\" qualifier is). Since it cannot change during the
...
-   We evaluate the calls to
    ...
    and
    [ta.crossunder()](https://www.tradingview.com/pine-script-reference/v5/#fun_ta%7Bdot%7Dcrossunder)
