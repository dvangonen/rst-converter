...
Pine Script™\'s `time series <PageTimeSeries>` and `type system <PageTypeSystem>`. Understanding all three is key to making the most of the
...
the
`events triggering the execution of a script <PageExecutionModel_Events>` has occurred.
...
-   The built-in `barstate.*` variables provide information on
    `the type of bar or the event <PageBarStates>` where the script is executing. The page where they are
    ...
    example.
-   The `Strategies <PageStrategies>` page
